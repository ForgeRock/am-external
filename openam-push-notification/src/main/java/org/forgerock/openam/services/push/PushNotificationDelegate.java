...
/**
 * A {@link PushNotificationDelegate} is an implementation of OpenAM's Push Notification Service
 * {@link PushNotificationService} specific to a realm as generated by a {@link PushNotificationDelegateFactory}.
 *
 * It is used for sending messages out via a Push service provider (such as Amazon's SNS) via {@link #send(PushMessage)}
 * and provides methods to retrieve its service for replies to respond at via {@link #getAuthServiceLocation()}
 * and {@link #getRegServiceLocation()}. Response messages should be handled by an endpoint or service and may use
 * {@link PushMessageResource} to assist in ensuring messages return to the appropriate sender (either via local
 * memory of via the CTS if available). A delegate should make its {@link Predicate}s available via the appropriate
 * {@link #getAuthenticationMessagePredicates()} and {@link #getRegistrationMessagePredicates()} and ensure they are
 * evaluated prior to response messages being returned to the appropriate place.
 *
 ...
 * by calling {@link #updateDelegate(PushNotificationServiceConfig)}.
 *
 * As of OpenAM 13.5, a default Amazon SNS-based endpoint {@link SnsMessageResource} is supplied with the system and
 ...
 *
 * If a new exposed service or CREST endpoint is added, the location of this should be advertised via the
 * {@link #getRegServiceLocation()} and {@link #getAuthServiceLocation()} methods.
 *
 * @supported.all.api
 */
