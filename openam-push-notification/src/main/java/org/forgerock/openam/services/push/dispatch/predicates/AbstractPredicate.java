/*
 * The contents of this file are subject to the terms of the Common Development and
 * Distribution License (the License). You may not use this file except in compliance with the
 * License.
 *
 * You can obtain a copy of the License at legal/CDDLv1.0.txt. See the License for the
 * specific language governing permission and limitations under the License.
 *
 * When distributing Covered Software, include this CDDL Header Notice in each file and include
 * the License file at legal/CDDLv1.0.txt. If applicable, add the following below the CDDL
 * Header, with the fields enclosed by brackets [] replaced by your own identifying
 * information: "Portions copyright [year] [name of copyright owner]".
 *
 * Copyright 2016-2019 ForgeRock AS.
 */
package org.forgerock.openam.services.push.dispatch.predicates;

import org.forgerock.guice.core.InjectorHolder;
import org.forgerock.am.cts.utils.JSONSerialisation;

import com.fasterxml.jackson.annotation.JsonIgnore;

/**
 * An abstract implementation of the predicate, holding a reference to the class used
 * for serialization. Extended by all predicate implementations to allow them to be replicated
 * across the CTS as necessary.
 */
public abstract class AbstractPredicate implements Predicate {

    /**
     * Used to serialize extending classes.
     */
    @JsonIgnore
    protected final JSONSerialisation jsonSerialisation = InjectorHolder.getInstance(JSONSerialisation.class);

    @Override
    public String jsonify() {
        return jsonSerialisation.serialise(this);
    }

}
