...
public class MessageDispatcher {
     ...
     */
    public void handle(String messageId, JsonValue content, ClusterMessageHandler messageHandler)
        ...
        if (messagePromise != null) {
            try {
                tryHandleFromLocalCache(messagePromise, content, messageId);
            } catch (CoreTokenException e) {
                debug.warning("Error deleting the cluster-wide message {} from the CTS.", messageId);
            }
        } else {
            try {
                tryFromCTS(content, messageId, messageHandler);
            } catch (CoreTokenException | ClassNotFoundException e) {
                debug.warning("Cache was asked to handle {} but could not utilises the CTS.", messageId);
     ...
     */
    public boolean forget(String messageId) throws CoreTokenException {
        MessagePromise messagePromise = cache.getIfPresent(messageId);
        ...
        ctsPersistentStore.deleteAsync(messageId);
    ...
    public MessagePromise expect(String messageId, Set<Predicate> predicates)
        ...
        MessagePromise messagePromise =
                new MessagePromise(PromiseImpl.<JsonValue, Exception>create(), predicates, messageId);
        getAuditRequestContext().addTrackingId(messagePromise.getAuditTrackingId());
    ...
    private void tryHandleFromLocalCache(MessagePromise messagePromise, JsonValue content, String messageId)
    ...
    private void tryFromCTS(JsonValue content, String messageId, ClusterMessageHandler messageHandler)
            throws CoreTokenException, NotFoundException, ClassNotFoundException, PredicateNotMetException {
        Token coreToken = ctsPersistentStore.read(messageId);
    ...
    private void storeInCTS(MessagePromise messagePromise, int timeout) throws CoreTokenException {
        Token ctsToken = new Token(messagePromise.getMessageId(), TokenType.PUSH);
