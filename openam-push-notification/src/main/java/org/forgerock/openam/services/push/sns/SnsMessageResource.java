...
/**
 * Amazon SNS implementation of a CREST endpoint for the Push Notification Service.
 *
 * Decouples the internal operations relating to using the message dispatcher from the endpoint itself by using the
 * internal {@link PushMessageResource}. This would allow for this class to perform message manipulation if
 * desired, prior to being handled by OpenAM.
 */
...
public class SnsMessageResource {
     ...
     * attempt to resolve it against an {@link MessageDispatcher#expect(String, Set)}'d message.
     *
    ...
    public Promise<ActionResponse, ResourceException> authenticate(Context context, ActionRequest actionRequest) {
        return pushMessageResource.handle(context, actionRequest, PushMessageResource.RequestType.AUTHENTICATE);
    }
    ...
    public Promise<ActionResponse, ResourceException> register(Context context, ActionRequest actionRequest) {
        return pushMessageResource.handle(context, actionRequest, PushMessageResource.RequestType.REGISTER);
    }
