...
public class SnsClientFactory {
     ...
     */
    public AmazonSNSClient produce(PushNotificationServiceConfig.Realm config) {
        AmazonSNSClient service = new AmazonSNSClient(
                new BasicAWSCredentials(config.accessKey(), new String(config.secret())));
        service.setRegion(Region.getRegion(Regions.fromName(config.region())));
        return service;
    }
