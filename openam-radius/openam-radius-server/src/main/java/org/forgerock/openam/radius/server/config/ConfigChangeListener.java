...
public class ConfigChangeListener implements ServiceListener {
    ...
    public void startListening() {
        logger.debug("Entering ConfigChangeListener.startListening()");
        ...
        logger.debug("Exiting ConfigChangeListener.startListening");
    }
    ...
    public String waitForConfigChange() throws InterruptedException {
        logger.debug("Entering ConfigChangeListener.waitForConfigChange()");
        String msg = null;
        try {
            msg = configChangedQueue.take();
        } catch (final InterruptedException e) {
            logger.warn("Thread waiting for config change has been interrupted.");
            throw e;
        }
        logger.debug("Exiting ConfigChangeListener.waitForConfigChange, returning " + msg);
        return msg;
    }
    ...
    public void globalConfigChanged(String serviceName, String version, String groupName, String serviceComponent,
            int type) {
        logger.debug("Entering globalConfigChange()");
        final boolean accepted = configChangedQueue.offer("RADIUS Config Changed. Loading...");

        if (!accepted) {
            logger.debug("RADIUS Client handlerConfig change event ignored.");
        }
        logger.debug("Leaving globalConfigChange()");
    }
