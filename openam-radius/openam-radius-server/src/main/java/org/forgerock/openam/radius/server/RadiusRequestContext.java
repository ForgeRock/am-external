...
public class RadiusRequestContext {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(RadiusRequestContext.class);
    ...
    public void logPacketContent(Packet pkt, String preamble) {
        LOG.warn(preamble + "\n" + Utils.getPacketRepresentation(pkt));
    }
    ...
    public void send(Packet response) throws RadiusProcessingException {
        if (sendWasCalled) {
            LOG.warn("Handler class '" + clientConfig.getAccessRequestHandlerClass().getSimpleName()
                    + "' declared for client "
