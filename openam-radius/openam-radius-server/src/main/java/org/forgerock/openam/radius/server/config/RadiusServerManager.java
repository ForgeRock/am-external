 ...
 * Portions Copyright 2019 ForgeRock AS.
 */
...
public final class RadiusServerManager implements Runnable, RadiusServer {
    ...
    private static final Logger logger = LoggerFactory.getLogger(RadiusServerManager.class);
    ...
    public RadiusServerManager(ConfigLoader configLoader, ConfigChangeListener configChangeListener,
        ...
        logger.debug("Constructing RadiusServiceStarter");
    ...
    public void shutdown() {
        logger.debug(this.getClass().getSimpleName() + "#shutdown() method called.");
        final Thread coordinator = coordinatingThread;

        if (coordinator != null) {
            final String name = coordinator.getName();
            logger.debug(this.getClass().getSimpleName() + " interrupting " + name);
            coordinator.interrupt();

            while (coordinatingThread != null) {
                logger.debug("Waiting for " + name + " to exit.");
                try {
                    Thread.sleep(200);
                } catch (final InterruptedException e) {
                    logger.debug("Thread sleep was interupted.");
                    Thread.currentThread().interrupt();
                }
            }
            logger.debug("Thread " + name + " server shutdown.");
        }
    }
    ...
    public synchronized void startUp() {
        if (coordinatingThread == null) {
            ...
            logger.warn(this.getClass().getSimpleName() + ".setServletConfig() called again. Service "
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                logger.debug(changeMsg);

                final RadiusServiceConfig cfg = configLoader.loadConfig();
                if (cfg != null) {
                    logger.debug("New RADIUS configuration loaded." + cfg);
                } else {
                    logger.warn("Unable to load new RADIUS configuration. Ignoring change.");
            ...
            logger.warn(Thread.currentThread().getName() + " interrupted. Exiting.");
        ...
        logger.warn(Thread.currentThread().getName() + " exited.");
        this.coordinatingThread = null;
    }
