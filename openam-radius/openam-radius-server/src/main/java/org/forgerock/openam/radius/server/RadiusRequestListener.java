...
public class RadiusRequestListener implements Runnable {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(RadiusRequestListener.class);
    ...
    public RadiusRequestListener(final RadiusServiceConfig config,
        ...
        try {
            ...
            LOG.debug("Starting RADIUS listener on port " + Integer.toString(radiusPort));
    ...
    public void terminate() {
        ...
        while (!finished) {
            try {
                LOG.warn("Waiting for RADIUS thread pool's request handler(s) to finish processing.");
                finished = executorService.awaitTermination(RadiusServerConstants.THREAD_POOL_SHUTDOWN_WAIT_SECONDS,
                        TimeUnit.SECONDS);
            } catch (final InterruptedException e) {
                LOG.warn("InterruptedException caught while waiting for executorService to terminate.");
                interrupted = true;
            }
        }
        ...
        if (t != null) {
            ...
            while (listenerThread != null) {
                LOG.warn("Waiting for RADIUS Listener to exit.");
                try {
                    Thread.sleep(200);
                } catch (final InterruptedException e) {
    ...
    public void run() {
            ...
            try {
                ...
                try {
                    ...
                    if (iAddr == null) {
                        ...
                        LOG.debug("DatagramChannel receive returned null. No datagram available.");
                        continue;
                    } else {
                    ...
                    LOG.warn("Exception Receiving RADIUS packet. Ignoring.", e);
                    continue;
                ...
                if (clientConfig == null) {
                    LOG.warn("No Defined RADIUS Client matches IP address " + ipAddr + ". Dropping request.");
                    eventBus.post(new PacketDroppedSilentlyEvent());
                    continue;
                }
                if (!clientConfig.isClassIsValid()) {
                    LOG.warn("Declared Handler Class for Client '" + clientConfig.getName()
        ...
        LOG.debug("RADIUS Listener Exited.");
