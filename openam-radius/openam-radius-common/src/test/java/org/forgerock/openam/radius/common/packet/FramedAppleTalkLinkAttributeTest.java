...
import static org.forgerock.openam.radius.common.AttributeType.FRAMED_APPLETALK_LINK;

import org.junit.jupiter.api.Test;

/**
 * Tests for {@link FramedAppleTalkLinkAttribute}.
 */
public class FramedAppleTalkLinkAttributeTest {

    @Test
    void testUnnumbered() {
        final FramedAppleTalkLinkAttribute a = new FramedAppleTalkLinkAttribute(0);
        assertThat(a.isUnumberedLink()).isTrue().describedAs("0 should result in unnumbered link");
        final byte[] bytes = a.getOctets();
        assertThat(bytes[0]).isEqualTo((byte) FRAMED_APPLETALK_LINK.getTypeCode());
    ...
    void testMaxNumbered() {
        ...
        assertThat(a.isUnumberedLink()).isFalse().describedAs(">0 should result in numbered link");
        final byte[] bytes = a.getOctets();
        assertThat(bytes[0]).isEqualTo((byte) FRAMED_APPLETALK_LINK.getTypeCode());
        assertThat(bytes[1]).isEqualTo((byte) 6);
    ...
    void testMaxFromRawBytes() {
        ...
        assertThat(a.getNetworkNumber()).isEqualTo(65535);
        assertThat(a.isUnumberedLink()).isFalse();
    }
    ...
    void testUnnumberedFromRawBytes() {
        ...
        final FramedAppleTalkLinkAttribute a = new FramedAppleTalkLinkAttribute(raw);
        assertThat(a.getNetworkNumber()).isEqualTo(0);
        assertThat(a.isUnumberedLink()).isTrue();
    }
}
