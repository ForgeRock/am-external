...
public class UtilProxyIDPRequestValidator implements IDPRequestValidator {
    private final Logger debug = LoggerFactory.getLogger(UtilProxyIDPRequestValidator.class);
     ...
     */
    public UtilProxyIDPRequestValidator(String reqBinding, boolean isFromECP, SAML2MetaManager saml2MetaManager) {
        ...
        debug.debug("Using request binding: {}", reqBinding);
    }
    ...
    public String getMetaAlias(HttpServletRequest request) throws ClientFaultException {
        ...
        debug.debug("Checking for Meta Alias in Parameter: {}", r);
        if (StringUtils.isBlank(r)) {
            r = SAML2MetaUtils.getMetaAliasByUri(request.getRequestURI());
            debug.debug("Checking for Meta Alias in URI: {}", r);
        }
        if (StringUtils.isBlank(r)) {
    ...
    public IDPAdapter getIDPAdapter(String realm, String idpEntityID) {
        ...
        debug.debug("Using IDP Adapter class: {}", r.getClass().getSimpleName());
        return r;
    }
