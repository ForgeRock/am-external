...
public class UtilProxySAMLAuthenticator extends SAMLBase implements SAMLAuthenticator {
    ...
    private static final Logger logger = LoggerFactory.getLogger(UtilProxySAMLAuthenticator.class);
    @VisibleForTesting
    ...
    public void authenticate() throws FederatedSSOException, IOException {
        ...
        try {
            ...
            logger.error("Unable to serialize the authentication request", saml2ex);
        ...
        if (!SAML2Utils.isSourceSiteValid(authnRequest.getIssuer(), data.getRealm(), data.getIdpEntityID())) {
            logger.warn("Issuer in Request is not valid.");
            throw new ClientFaultException(data.getIdpAdapter(), INVALID_SAML_REQUEST);
        }
        ...
        try {
            ...
            logger.error("Unable to get IDP SSO Descriptor from meta.");
        ...
        try {
            ...
            logger.error("Unable to get SP SSO Descriptor from meta.", sme);
        }

        if (spSSODescriptor == null) {
            logger.error("Unable to get SP SSO Descriptor from meta.");
            throw new ServerFaultException(data.getIdpAdapter(), METADATA_ERROR);
        }
        ...
        if (idpSSODescriptor.isWantAuthnRequestsSigned() || spSSODescriptor.isAuthnRequestsSigned()) {
            ...
            try {
                ...
                if (!isSignatureOK) {
                    logger.error("authn request verification failed.");
                ...
                if (!isFromECP) {
                    ...
                    if (endPoint == null || StringUtils.isEmpty(endPoint.getLocation())) {
                        logger.error("authn request unable to get endpoint location for IdpEntity: {}  MetaAlias: {} ",
                                data.getIdpEntityID(), data.getIdpMetaAlias());
                    ...
                    if (!SAML2Utils.verifyDestination(authnRequest.getDestination(), endPoint.getLocation())) {
                        logger.error("authn request destination verification failed for IdpEntity: {}  MetaAlias: {} "
                ...
                logger.error("authn request verification failed.", se);
                throw new ClientFaultException(data.getIdpAdapter(), "invalidSignInRequest");
            }
        ...
        if (data.getRequestID() == null) {
            logger.error("SAML Authentication Request id is null");
        ...
        if (isFromECP) {
            try {
                ...
                logger.debug("Unable to retrieve user session.");
            }
        } else {
        ...
        if (idpAuthnContextInfo == null) {
            logger.debug("Unable to find valid AuthnContext. Sending error Response.");
            try {
                ...
                logger.error("An error occurred while creating error response", sme);
                throw new ServerFaultException(data.getIdpAdapter(), METADATA_ERROR);
            }
            return;
        }
        ...
        if (data.getSession() == null) {
            ...
            if (isValidSessionInRealm) {
                ...
                logger.debug("IDP Session Upgrade is: {}", sessionUpgrade);
            }
            ...
            if (sessionUpgrade || !isValidSessionInRealm ||
                ...
                if (isValidSessionInRealm) {
                    try {
                        ...
                        if (isProxy) {
                            ...
                            if (preferredIDP != null) {
                                if ((SPCache.reqParamHash != null)
                                    ...
                                    logger.debug("IDP to be proxied {}", preferredIDP);
                                    IDPProxyUtil.sendProxyAuthnRequest(authnRequest, preferredIDP,
                        ...
                        logger.debug("Redirecting for the proxy handling error: {}", re.getMessage());
                ...
                try { //and they want to authenticate
                    if (!Boolean.TRUE.equals(authnRequest.isPassive())) {
                        ...
                        try { //and they want to get into the system with passive auth - response no passive
                            ...
                            logger.error("An error occurred while sending back NoPassive response", sme);
                            redirectException = new ServerFaultException(data.getIdpAdapter(), METADATA_ERROR);
                        }
                    }
                } catch (IOException ex) {
                    logger.error("Unable to redirect to authentication.", ex);
    ...
    private void redirectToAuth(SPSSODescriptorType spSSODescriptor, String binding,
        ...
        try {
            ...
            if (isProxy) {
                ...
                if (preferredIDP != null) {
                    if ((SPCache.reqParamHash != null) && (!(SPCache.reqParamHash.containsKey(preferredIDP)))) {
                        ...
                        IDPProxyUtil.sendProxyAuthnRequest(data.getAuthnRequest(), preferredIDP,
                                data.getIdpEntityID(), request, response, data.getRealm(),
                                data.getRelayState(), binding);
                        return;
                    } else {
            ...
            logger.debug("{} Redirecting for the proxy handling error: {}", classMethod, re.getMessage());
        ...
        try {
            ...
            logger.error(classMethod + "Unable to redirect to authentication.", e);
    ...
    private static AuthnRequest getAuthnRequest(String compressedReq) {
        ...
        if (outputString != null) {
            try {
                ...
                logger.error("UtilProxySAMLAuthenticator.getAuthnRequest(): cannot construct a AuthnRequest "
    ...
    private static AuthnRequest getAuthnRequest(HttpServletRequest request, boolean isFromECP, String binding) {
        ...
        if (isFromECP) {
            ...
            if (samlRequest == null) {
                logger.error("UtilProxySAMLAuthenticator.getAuthnRequest: SAMLRequest is null");
                return null;
            }
            if (binding.equals(SAML2Constants.HTTP_REDIRECT)) {
                logger.debug("UtilProxySAMLAuthenticator.getAuthnRequest: saml request = {}", samlRequest);
                ...
                try {
                    ...
                    if (raw != null) {
                        ...
                        if (doc != null) {
                            logger.debug("UtilProxySAMLAuthenticator.getAuthnRequest: decoded SAML2 Authn "
                                    + "Request: {}",
                                    XMLUtils.print(doc.getDocumentElement()));
                            authnRequest = ProtocolFactory.getInstance().createAuthnRequest(doc.getDocumentElement());
                        } else {
                            logger.error("UtilProxySAMLAuthenticator.getAuthnRequest: Unable to parse "
