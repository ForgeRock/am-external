...
public class ValidRelayStateExtractor implements ValidDomainExtractor<ValidRelayStateExtractor.SAMLEntityInfo> {
    ...
    private static final Logger DEBUG = LoggerFactory.getLogger(ValidRelayStateExtractor.class);
    ...
    public Collection<String> extractValidDomains(final SAMLEntityInfo entityInfo) {
        try {
            ...
            if (config == null) {
                DEBUG.warn("ValidRelayStateExtractor.getValidDomains: Entity config is null for entityInfo: "
                        + entityInfo);
                return null;
            }
            attrs = SAML2MetaUtils.getAttributes(config);
            if (attrs == null) {
                DEBUG.warn("ValidRelayStateExtractor.getValidDomains: Cannot find extended attributes");
                return null;
            }
            ...
            DEBUG.warn("Unable to retrieve extended configuration", sme);
        }
        return null;
    }
