...
public class Action {
    ...
    public Action(Element element) throws SAMLException{
        ...
        if (element == null) {
            logger.debug("Action: Input is null.");
        ...
        if ((tag == null) || (!tag.equals("Action"))) {
            logger.debug("Action: wrong input");
            throw new SAMLRequesterException(
            ...
            if (att.getNodeType() == Node.ATTRIBUTE_NODE) {
                ...
                if (attName == null || attName.length() == 0) {
                    if (logger.isDebugEnabled()) {
                        logger.debug("Action: Attribute Name" +
    ...
    public Action(String namespace, String action) throws SAMLException {
        if (namespace == null || namespace.length() == 0) { 
            if (logger.isDebugEnabled()) {
                logger.debug("Action:Take default " + 
                                              "Attribute Namespace.");
            }
        } else {
