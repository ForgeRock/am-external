...
public class SOAPCommunicator {
    ...
    public SOAPMessage createSOAPMessage(final String header,
        ...
        try {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("SOAPCommunicator.createSOAPMessage: header = " +
                        header + ", body = " + body);
            }
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("SOAPCommunicator.createSOAPMessage: soap message = " +
                        sb.toString());
            }
    ...
    public Element getSOAPBody(final SOAPMessage message)
        ...
        debug.debug("SOAPCommunicator.getSOAPBody : start");
        ...
        Document doc = XMLUtils.toDOMDocument(byteArrayIn);
        Element root = doc.getDocumentElement();
        if (debug.isDebugEnabled()) {
            debug.debug("SOAPCommunicator.getSOAPBody : soap body =\n"
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (child.getNodeType() != Node.ELEMENT_NODE) {
                debug.debug("SOAPCommunicator.getSOAPBody: " + child);
                continue;
            }
            String childName = child.getLocalName();
            if (debug.isDebugEnabled()) {
                debug.debug("SOAPCommunicator.getSOAPBody: local name= " + childName);
            }
    ...
    public Element getSamlpElement(final SOAPMessage message,
        ...
        for (int i = 0; i < childNodeLength; i++) {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("SOAPCommunicator.getSamlpElement: node={}, nsURI={}",
