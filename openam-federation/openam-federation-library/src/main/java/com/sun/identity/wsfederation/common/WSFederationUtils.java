...
public class WSFederationUtils {    
    ...
    private static Logger debug = 
        LoggerFactory.getLogger(WSFederationUtils.class);
    /**
        ...
        String accountRealmCookieName )
    {
        ...
        if ( leftBracket == -1 ) {
            if (debug.isWarnEnabled()) {
                debug.warn(classMethod + "Can't find left bracket");
            }
            return null;
        }
        ...
        if ( rightBracket == -1 || rightBracket < leftBracket ) {
            if (debug.isWarnEnabled()) {
                debug.warn(classMethod + "Can't find right bracket");
            }
            return null;
        }
        ...
        if ( insideBrackets.length() == 0 ) {
            if (debug.isWarnEnabled()) {
                debug.warn(classMethod + "zero length between brackets");
            }
            return null;
        }
        ...
        for ( int i = 0; i < uaFields.length; i++ ) {
            if ( uaFields[i].indexOf(accountRealmCookieName) != -1 ) {
                ...
                if ( ! keyValue[0].equals(accountRealmCookieName)) {
                    if (debug.isWarnEnabled()) {
                        debug.warn(classMethod + "can't understand " + 
                            uaFields[i]);
                    }
                    return null;
                }
    ...
    public static void processMultiProtocolLogout(HttpServletRequest request,
        ...
        try {
            ...
            // ignore;
            debug.debug("WSFederationUtils.processMultiProtocolLogout", ex);
    ...
    public static boolean isWReplyURLValid(HttpServletRequest request, String relayState) {
        ...
        try {
            ...
            debug.warn("Can't get metaManager.", e);
            return false;
        }
    }
        ...
        if (metaAlias != null) {
            ...
            try {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("WSFederationUtils.isWReplyURLValid(): wreply " + wreply +
    ...
    private static boolean isLogoutUrlValid(String url, String realm, HttpServletRequest request) {
        RedirectUrlValidator<String> validator =
                InjectorHolder.getInstance(Key.get(new TypeLiteral<RedirectUrlValidator<String>>(){}));
