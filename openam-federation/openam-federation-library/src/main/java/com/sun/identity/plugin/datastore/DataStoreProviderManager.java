...
public final class DataStoreProviderManager {
    ...
    private final Map<String, DataStoreProvider> providerMap = new HashMap<>();
    ...
    private Logger debug = LoggerFactory.getLogger(DataStoreProviderManager.class);
        ...
        throws DataStoreProviderException
    {
        if (StringUtils.isEmpty(componentName)) {
            ...
            DataStoreProvider provider = providerMap.get(componentName);
            if (provider != null) {
                return provider;
            }
            ...
            if (StringUtils.isNotEmpty(className)) {
                try {
                    provider = InjectorHolder.getInstance(Class.forName(className).asSubclass(DataStoreProvider.class));
                } catch (Exception e) {
                    debug.error("Exception while instantiating provider: {}", className, e);
                    throw new DataStoreProviderException(e);
                }
            }
            if (provider == null) {
                debug.debug("No provider specified for {}, using default.", componentName);
    ...
    private DataStoreProvider getDefaultProvider() throws DataStoreProviderException {
        ...
        if (StringUtils.isEmpty(className)) {
            throw new DataStoreProviderException(bundle.getString("defaultProviderNotDefined"));
        }
        try {
            return InjectorHolder.getInstance(Class.forName(className).asSubclass(DataStoreProvider.class));
        } catch (Exception e) {
            throw new DataStoreProviderException(e);
        }
    }
