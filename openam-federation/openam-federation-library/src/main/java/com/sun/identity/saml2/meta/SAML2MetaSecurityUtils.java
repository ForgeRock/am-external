...
import org.forgerock.openam.saml2.meta.Saml2MetaSecrets;
...
public final class SAML2MetaSecurityUtils {
    ...
    private static Logger debug = LoggerFactory.getLogger(SAML2MetaSecurityUtils.class);
    ...
    public static final String NS_ALG_SUPPORT = "urn:oasis:names:tc:SAML:metadata:algsupport";
    ...
    public static final String PREFIX_ALG = "alg";
    ...
    public static Document sign(String realm, EntityDescriptorElement descriptor)
        ...
        Document doc = XMLUtils.toDOMDocument(xmlstr);
        ...
        try {
            ...
            Saml2MetaSecrets saml2MetaSecrets = new Saml2MetaSecrets(secrets);
            SigningKey signingKey = saml2MetaSecrets.getSigningKey(realm);
            signatureProvider.signXmlUsingPrivateKey(doc, saml2MetaSecrets.getPrivateKey(signingKey),
                    saml2MetaSecrets.getX509Certificate(signingKey), descriptorId, xpath);
        } catch (XMLSignatureException xmlse) {
            if (debug.isDebugEnabled()) {
                debug.debug("SAML2MetaSecurityUtils.sign:", xmlse);
            }
        }
        ...
        throws SAML2MetaException
    {
        ...
        try {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("SAML2MetaSecurityUtils.verifySignature:", ex);
                throw new SAML2MetaException(ex.getMessage());
            }
        }
        int numSigs = sigElements.getLength();
        if (debug.isDebugEnabled()) {
            debug.debug("SAML2MetaSecurityUtils.verifySignature:" +
