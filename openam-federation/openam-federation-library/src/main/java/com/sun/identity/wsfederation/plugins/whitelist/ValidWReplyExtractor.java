...
public class ValidWReplyExtractor implements ValidDomainExtractor<ValidWReplyExtractor.WSFederationEntityInfo> {
    ...
    private static final Logger DEBUG = LoggerFactory.getLogger(ValidWReplyExtractor.class);
    ...
    public Collection<String> extractValidDomains(final WSFederationEntityInfo entityInfo) {
        try {
            ...
            if (config == null) {
                DEBUG.warn("ValidWReplyExtractor.getValidDomains: Entity config is null for entityInfo: "
                        + entityInfo);
                return null;
            }
            ...
            if (attrs == null) {
                DEBUG.warn("ValidWReplyExtractor.getValidDomains: Cannot find extended attributes");
                return null;
            }
