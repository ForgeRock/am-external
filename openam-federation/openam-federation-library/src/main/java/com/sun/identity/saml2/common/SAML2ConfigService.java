...
public class SAML2ConfigService implements ConfigurationListener {
    ...
    private static Logger debug = LoggerFactory.getLogger(SAML2ConfigService.class);
    ...
    public void configChanged(ConfigurationActionEvent e) {
        if (debug.isDebugEnabled()) {
            debug.debug("SAML2ConfigService: configChanged");
        }
        setValues();
    }
    ...
    static private synchronized void setValues() {
        if (ci == null) {
            ...
            debug.warn("ConfigurationInstance is null, so default value for buffer length (2048) will be set.");
        } else {
            Map attrMap = null; 
            try {
               attrMap = ci.getConfiguration(null, null);
            } catch (ConfigurationException ce) {
               debug.error("Exception caught obtaining updated configuration. " + SAML2_BUFFER_LENGTH +
                       " will not be updated. Exception: " + ce, ce);
               return;
            }
            Map newAttributes = new HashMap();
            if (attrMap != null) {
                if (debug.isDebugEnabled()) {
                    debug.debug("Configuration updated");
                }
                Set values = (Set)attrMap.get(SAML2_BUFFER_LENGTH);
                String value = "2048" ;
                if ((values != null) && (values.size() == 1)) {
                     value = (String) values.iterator().next(); 
                } else {
                    debug.warn("Value for " + SAML2_BUFFER_LENGTH + " null or size!=1. Defaulting to 2048.");
                }
                newAttributes.put(SAML2_BUFFER_LENGTH, value);                 
            } else {
                debug.warn("Attribute map returned from ConfigurationInstance for the SAML2 config is null! " +
                        "Default value for buffer length (2048) will be set.");
            }
            attributes = newAttributes;
        } 
        if (debug.isDebugEnabled()) {
            debug.debug("Attributes in SAML2ConfigService updated to: "
                + attributes);
        }
    }
