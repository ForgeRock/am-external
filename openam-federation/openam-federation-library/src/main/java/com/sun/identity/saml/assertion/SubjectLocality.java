...
public class SubjectLocality {
    private static final Logger logger = LoggerFactory.getLogger(SubjectLocality.class);
        ...
        throws SAMLException
    {
        ...
        if (eltName == null)  {
            if (logger.isDebugEnabled()) {
                logger.debug("SubjectLocality: local name "
                        + "missing");
            }
    ...
    public boolean setDNSAddress(java.lang.String dnsAddress) {
        if ((dnsAddress == null) || (dnsAddress.length() == 0)) {
            logger.debug("DNS Address is null");
            return false;
        }
        _dnsAddress = dnsAddress;
        return true;
    ...
    public boolean setIPAddress(java.lang.String ipAddress) {
        if ((ipAddress == null) || (ipAddress.length() == 0)) {
            logger.debug("IP Address is null");
            return false;
        }
        _ipAddress = ipAddress;
        return true;
