...
public class ArtifactImpl implements Artifact {
	...
	// make sure that the input xml block is not null
        if (element == null) {
            if (logger.isDebugEnabled()) {
                logger.debug("ArtifactImpl.parseElement: "
                    + "Input is null.");
            }
	...
	if (value == null || value.length() == 0) {
	    if (logger.isDebugEnabled()) {
		logger.debug(method + "empty input.");
	    }
            throw new SAML2Exception(
                SAML2SDKUtils.bundle.getString("nullInput"));
	}
	...
	// decode the artifact
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("ArtifactImpl.parseElement: exception "
        ...
        if (raw.length != ARTIFACT_4_LENGTH) {
            if (logger.isDebugEnabled()) {
                logger.debug("ArtifactImpl.parseElement: the length"
	...
	// check if the typecode is correct
	    ...
	    if (logger.isDebugEnabled()) {
		logger.debug("ArtifactImpl.parseElement: wrong "
		    + "typecode.");
	    }
            throw new SAML2Exception(
                SAML2SDKUtils.bundle.getString("wrongInput"));
        ...
        try {
            ...
            logger.error("ArtifactImpl.parseElement: encoding "
		+ "exception: ", e);
	...
	if (typecode != null) {
	    ...
	    {
		logger.error("ArtifactImpl: wrong typecode.");
		throw new SAML2Exception(
		    SAML2SDKUtils.bundle.getString("wrongInput"));
	    }
	}
	...
	if (sourceid == null) {
            logger.error("ArtifactImpl: null sourceID.");
            throw new SAML2Exception(
                SAML2SDKUtils.bundle.getString("nullInput"));
        }
        sourceID = sourceid;
	...
	if (messagehandle == null) {
        ...
        try {
            ...
            logger.error("ArtifactImpl: encoding exception: ",e);
        ...
        if (idBytes.length != SAML2Constants.ID_LENGTH ||
            ...
            logger.error("ArtifactImpl: wrong input length.");
            throw new SAML2Exception(
                SAML2SDKUtils.bundle.getString("wrongInput"));
        }
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("ArtifactImpl: exception encode"
                        + " input:", e);
            }
            throw new SAML2Exception(
