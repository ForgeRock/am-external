...
import static com.sun.identity.wsfederation.common.WSFederationConstants.ACTIVE_REQUESTOR_PROFILE_ENABLED;
import static com.sun.identity.wsfederation.common.WSFederationConstants.AUTHENTICATOR_CLASS;
import static com.sun.identity.wsfederation.common.WSFederationConstants.BUNDLE_NAME;
import static com.sun.identity.wsfederation.common.WSFederationConstants.TRUSTED_ADDRESSES;
import static com.sun.identity.wsfederation.common.WSFederationConstants.WSA_NAMESPACE;
import static com.sun.identity.wsfederation.common.WSFederationConstants.WSSE_NAMESPACE;
import static com.sun.identity.wsfederation.common.WSFederationConstants.WST_NAMESPACE;
import static com.sun.identity.wsfederation.common.WSFederationConstants.WSU_NAMESPACE;
...
public class ActiveRequest extends WSFederationAction {
    ...
    private static final Logger DEBUG = LoggerFactory.getLogger(ActiveRequest.class);
    ...
    public void process() throws ServletException, IOException, WSFederationException {
        ...
        if (!activeRequestorEnabled) {
            DEBUG.warn("Active Requestor Profile is not enabled for the hosted IdP {}", idpEntityId);
        ...
        try (InputStream is = request.getInputStream()) {
            ...
            if (DEBUG.isDebugEnabled()) {
                ...
                DEBUG.debug("SOAP message received: " + maskPassword(new String(baos.toByteArray(),
            ...
            DEBUG.debug("An error occurred while processing the Active Request", are);
            ...
            if (ssoToken != null) {
                try {
                    ...
                    DEBUG.debug("Unable to invalidate temporary session", se);
                }
            }
        }
    ...
    private void parseAndValidateRequest(SOAPMessage soapMessage, IDPSSOConfigElement idpConfig) throws SOAPException,
        ...
        if (expires != null) {
            ...
            DEBUG.warn("The Expires attribute was not provided, can't validate if request has expired.");
        }
