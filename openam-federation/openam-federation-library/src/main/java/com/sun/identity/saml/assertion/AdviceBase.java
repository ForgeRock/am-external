...
public abstract class AdviceBase {
    private static final Logger logger = LoggerFactory.getLogger(AdviceBase.class);
    ...
    public AdviceBase(Element element) throws SAMLException {
        ...
        if (element == null) {
            logger.debug("Advice: null input.");
        ...
        if ((tag == null) || (!tag.equals("Advice"))) {
            logger.debug("Advice: wrong input");
            throw new SAMLRequesterException(
