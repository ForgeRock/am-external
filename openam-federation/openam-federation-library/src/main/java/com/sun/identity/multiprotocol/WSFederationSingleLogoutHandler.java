...
public class WSFederationSingleLogoutHandler implements SingleLogoutHandler {
    ...
    private static final Logger logger = LoggerFactory.getLogger(WSFederationSingleLogoutHandler.class);
    ...
    public int doIDPSingleLogout(
        ...
        logger.debug("WSFederationSingleLogoutHandler.doIDPSingleLogout : start");
        if (!isSessionUsedInWSFed(userSession, userID)) {
            // no session for this protocol
            logger.debug("WSFedSingleLogoutHandler.doIDPSLO : no action");
            return SingleLogoutManager.LOGOUT_NO_ACTION_STATUS;
        }

        if (isSOAPInitiated) {
            logger.debug("WSFedSLOHandler.doIDPSLO : SOAP initiated SLO");
            // WS-Federation does not support SOAP based profile
            return SingleLogoutManager.LOGOUT_FAILED_STATUS;
        } else {
            logger.debug("WSFedSLOHandler.doIDPSLO : HTTP initiated SLO");
            ...
            logger.debug("WSFedSLOHandler.doIDPSLO: HTTPinit, redirect to {}", redirectURL);
    ...
    private String findIDPMetaAlias(String idpEntityID, String spEntityID,
        ...
        try {
            ...
            logger.debug("WSFedSingleLogoutHandler.findIDPMetaAlias: all hosted WS-Fed IDPs = {}", hostedIdps);
            ...
            for (int i = 0; i < num; i++) {
                ...
                while (it.hasNext()) {
                    ...
                    logger.debug("WSFedSLOHandler.findIDPMetaAlias: check COT = {}", cotName);
                    ...
                    logger.debug("WSFedSingleLogoutHandler.findIDPMetaAlias : found IDP {} in COT {}", idpId, cotName);
            ...
            logger.error("WSFederationSingleLogoutHandler.findIDPMetaAlias", e);
        }
        return null;
    }
