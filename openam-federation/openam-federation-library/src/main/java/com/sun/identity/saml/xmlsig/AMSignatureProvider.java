...
public class AMSignatureProvider implements SignatureProvider {
    private static final Logger logger = LoggerFactory.getLogger(AMSignatureProvider.class);
    ...
    public AMSignatureProvider() {
        ...
        try {
            ...
            logger.error("AMSignatureProvider: " +
                "constructor error");
        }
            ...
            if (valCert != null)  {
                if (valCert.trim().equalsIgnoreCase("off")) {
                    ...
                    if(logger.isDebugEnabled()) {
                       logger.debug("SystemConfigurationUtil:"
    ...
    public void initialize(KeyProvider keyProvider) {
        if (keyProvider == null) {
            logger.error("Key Provider is null");
        } else {
    ...
    public org.w3c.dom.Element signXML(org.w3c.dom.Document doc,
        ...
        if (certAlias == null || certAlias.length() == 0) {
	        logger.error("signXML: certAlias is null.");
        ...
        return signXmlUsingPrivateKey(doc, privateKey, x509Certificate, algorithm, null,
    ...
    public java.lang.String signXML(java.lang.String xmlString,
	    ...
	    logger.error("signXML: xmlString is null.");
	    throw new XMLSignatureException(
		      SAMLUtilsCommon.bundle.getString("nullInput"));
	}
        if (certAlias == null || certAlias.length() == 0) {
        ...
        try {
            ...
            logger.error("signXML Exception: ", e);
    ...
    public java.lang.String signXML(java.lang.String xmlString,
        ...
        Document doc = null;
        try {
            doc = XMLUtils.toDOMDocument(xmlString);
        } catch (Exception e) {
            logger.error("signXML Exception: ", e);
            throw new XMLSignatureException(e.getMessage());
        }
    ...
    public Element signXMLUsingKeyPass(Document doc,
        ...
        if (includeCert) {
            x509Certificate = (X509Certificate) keystore.getX509Certificate(certAlias);
        }
        return signXmlUsingPrivateKey(doc, privateKey, x509Certificate, algorithm, idAttrName, ImmutableList.of(id), xpath, null, false);
    }

    /**
     * Sign part of the XML document referred by the supplied id attribute
     * using enveloped signatures and use exclusive XML canonicalization.
     * @param doc the XML dom object
     * @param privateKey the private key used for signing
     * @param x509Certificate the x509 certificate counter part of the signing private key
     * @param id the id attribute value of the node to be signed
     * @param xpath the expression to identify a node before which to insert the Signature element
     * @return the dom object with Signature
     * @throws XMLSignatureException if the document could not be signed
     */
    public Element signXmlUsingPrivateKey(Document doc,
            PrivateKey privateKey,
            X509Certificate x509Certificate,
            String id,
            String xpath) throws XMLSignatureException {
        return signXmlUsingPrivateKey(doc, privateKey, x509Certificate, null,
                SAML2Constants.ID, ImmutableList.of(id), xpath, null, false);
    }
     ...
     * @param certAlias Signer's certificate alias name
     * @param algorithm XML signature algorithm
     * @param transformAlg XML siganture transform algorithm
    ...
    public org.w3c.dom.Element signXML(org.w3c.dom.Document doc,
        ...
        X509Certificate x509Certificate = (X509Certificate) keystore.getX509Certificate(certAlias);

        return signXmlUsingPrivateKey(doc, privateKey, x509Certificate, algorithm, null, ids, null, transformAlg, false);
    }
	...
	if (doc == null) {
            logger.error("signXML: doc is null.");
            throw new XMLSignatureException(
                      SAMLUtilsCommon.bundle.getString("nullInput"));
        }
        if (cert == null) {
            logger.error("signWithWSSSAMLTokenProfile: " +
					"Certificate is null");
            throw new XMLSignatureException(
                      SAMLUtilsCommon.bundle.getString("nullInput"));
        }
	if (assertionID == null) {
        ...
        try {
            ...
            logger.error("signWithWSSX509TokenProfile " +
                "Exception: ", e);
            throw new XMLSignatureException(e.getMessage());
        }

	if (logger.isDebugEnabled()) {
            logger.debug("SAML Signed doc = " +
    ...
    public boolean verifyXMLSignature(String wsfVersion, String certAlias,
        ...
        if (doc == null) {
            logger.error("verifyXMLSignature:" +
                " document is null.");
            throw new XMLSignatureException(
                SAMLUtilsCommon.bundle.getString("nullInput"));
        }
	    ...
	    if (logger.isDebugEnabled()) {
		logger.debug("verifyXMLSignature: " +
		...
		if (logger.isDebugEnabled ()) {
			...
			XMLUtils.print(sigElement));
                ...
                if (refUri == null || signedId == null || !refUri.substring(1).equals(signedId)) {
                    logger.error("Signature reference ID does not match with element ID");
	    ...
	    logger.error("verifyXMLSignature Exception: ", ex);
    ...
    public boolean verifyXMLSignature(org.w3c.dom.Element element)
        throws XMLSignatureException {
        if (element == null) {
	    logger.error("signXML: element is null.");
	    throw new XMLSignatureException(
		      SAMLUtilsCommon.bundle.getString("nullInput"));
	}
            ...
            if (pk!=null) {
                ...
                if (StringUtils.isEmpty(certAlias)) {
                    if (logger.isWarnEnabled()) {
                        logger.warn("Could not find a KeyInfo and certAlias was not defined");
                    }
                    return false;
                }
                if (logger.isDebugEnabled()) {
                    logger.debug("Could not find a KeyInfo, "
                        + "try to use certAlias");
                }
	...
	try {
            logger.debug("getWSSTTokenProfilePublicKey:"+
                " entering");
	    ...
	    Element reference = (Element) keyinfo.getElementsByTagNameNS(
			...
			pubKey = getPublicKeybyDSARSAkeyValue(doc, reference);
                ...
                logger.error("getWSSTokenProfilePublicKey:" +
    ...
    private Element signXmlUsingPrivateKey(Document doc,
            PrivateKey privateKey,
            X509Certificate x509Certificate,
            String algorithm,
            String idAttrName,
            ...
            String xpath,
            String transformAlg,
            boolean useCanonicalizationMethod) throws XMLSignatureException {
        ...
        try {
            ...
            if (ids.size() == 1) {
                root = (Element) XPathAPI.selectSingleNode(
                        doc, "//*[@" + idAttrName + "=\"" + ids.get(0) + "\"]");
            } else {
                root = doc.getDocumentElement();
            }

            if (root == null) {
                logger.error("signXML: could not resolve id attribute");
                throw new XMLSignatureException(
                        SAMLUtilsCommon.bundle.getString("invalidIDAttribute"));
            }

            // Set the ID attribute if idAttrName is not the default.
            if (idAttrName!= null && !idAttrName.equals(DEF_ID_ATTRIBUTE)) {
            ...
            if (size == 0) {
                ...
                for (int i = 0; i < size; ++i) {
                    Transforms transforms =  new Transforms(doc);
                    transforms.addTransform(transformAlg == null ? Transforms.TRANSFORM_ENVELOPED_SIGNATURE : transformAlg);
                    transforms.addTransform(
                            Transforms.TRANSFORM_C14N_EXCL_OMIT_COMMENTS);
                    String id = (String) ids.get(i);
                    if (logger.isDebugEnabled()) {
                    ...
                    sig.addDocument("#"+id, transforms, digestAlg);
                }
            }
