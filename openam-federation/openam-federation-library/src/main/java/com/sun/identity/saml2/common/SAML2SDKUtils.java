...
public class SAML2SDKUtils {
    ...
    private static Logger debug = LoggerFactory.getLogger(SAML2SDKUtils.class);
    private static Logger decryptDebug = LoggerFactory.getLogger(SAML2SDKUtils.class + ".decryptedAssertions");
            ...
            try {
                ...
                if (implClass != null && implClass.trim().length() != 0) {
                    ...
                    if (debug.isDebugEnabled()) {
                        debug.debug("SAML2SDKUtils.init: mapper for " + iName
                                + "=" + implClass);
                    }
    ...
    public static Object getObjectInstance(String iName, String value){
        ...
        if (implClass == null) {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("SAML2SDKUtils.getObjectInstance: new customized "
    ...
    public static String createSOAPMessageString(String xmlString)
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("SAML2Utils.createSOAPMessage: xmlstr = " +
                    xmlString);
        }
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("SAML2Utils.createSOAPMessage: soap message = " +
                    sb.toString());
        }
        return sb.toString();
    }
    ...
    public static void decodeXMLToDebugLog(String callerName, Element xmlElement) {
        if (decryptDebug.isDebugEnabled() && isSAMLDecryptionDebugEnabled()) {
            String xmlOutput = XMLUtils.print(xmlElement);
            decryptDebug.debug(callerName + "Decrypted xml element node:\n" +
