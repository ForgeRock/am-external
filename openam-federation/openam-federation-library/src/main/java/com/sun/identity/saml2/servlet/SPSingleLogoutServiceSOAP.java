...
public class SPSingleLogoutServiceSOAP extends HttpServlet {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SPSingleLogoutServiceSOAP.class);
    ...
    public void doPost(
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("SPSLOSOAP.doPost : uri =" +
            ...
            logger.error("SPSingleLogoutServiceSOAP", ex);
            SAMLUtils.sendError(req, resp, 
                HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
                "singleLogoutFailed", ex.getMessage());
            return;
        } catch (SOAPException soap) {
            logger.error("SPSingleLogoutServiceSOAP", soap);
            SAMLUtils.sendError(req, resp, 
                HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
    ...
    public SOAPMessage onMessage(
        ...
        logger.debug("SPSLOServiceSOAP.onMessage: starting");
        ...
        try {
            ...
            logger.error("SPSingleLogoutServiceSOAP.onMessage: " +
        ...
        if (logoutReq == null) {
            logger.error("SPSLOServiceSOAP.onMessage: null request");
            return SOAPCommunicator.getInstance().createSOAPFault(SAML2Constants.CLIENT_FAULT,
        ...
        if (loRes == null) {
            logger.error("SPSLOSOAP.onMessage: null LogoutResponse");
