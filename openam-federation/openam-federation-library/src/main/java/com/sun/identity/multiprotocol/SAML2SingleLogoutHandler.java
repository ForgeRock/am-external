...
public class SAML2SingleLogoutHandler implements SingleLogoutHandler {
    ...
    public int doIDPSingleLogout(
        ...
        logger.debug("SAML2SingleLogoutHandler.doIDPSingleLogout : start");
        if (!isSessionUsedInSAML2(userSession, userID)) {
            // no session for this protocol
            logger.debug("SAML2SingleLogoutHander.doIDPSLO : no action");
    ...
    private String findIDPMetaAlias(String idpEntityID, String spEntityID,
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("SAML2SingleLogoutHandler.findIDPMetaAlias: "
            ...
            for (int i = 0; i < num; i++) {
                ...
                while (it.hasNext()) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("SAML2SLOHandler.findIDPMetaAlias: "
                        + " check COT = " + cotName);
                    }
            ...
            logger.error("SAML2SingleLogoutHandler." +
                    "findIDPMetaAlias", e);
        }
        return null;
    }
    ...
    private int handleSOAPInitiatedSingleLogout(Set userSession, String userID,
        ...
        logger.debug("SAML2SingleLogoutHanlder: handleSOAPInitiatedSLO");
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("SAML2SLOHandler.handleSOAPSLO: " +
        ...
        if ((sessIndex == null) || (sessIndex.length == 0)) {
            if (logger.isWarnEnabled()) {
                logger.warn("SAML2SLOHandler.handleSOAPSLO: " +
        ...
        if (idpSession == null) {
            logger.error("SAML2SLOHanlder.handleSOAPSLO: " +
        ...
        int n = list.size();
        if (logger.isDebugEnabled()) {
            logger.debug("SAML2SLOHanlder.handleSOAPSLO: " +
        ...
        for (int i = 0; i < n; i++) {
            ...
            try {
                ...
                logger.error("SAML2SLOHandler:handleSOAPSLO.doLogout" , ex);
                soapFailCount++;
                continue;
            }
        }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("SAML2SLOHandler.doSOAPSLO: return status for "  +
