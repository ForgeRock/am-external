...
public class DefaultSPAttributeMapper extends DefaultAttributeMapper 
    ...
    private static final Logger logger = LoggerFactory.getLogger(DefaultSPAttributeMapper.class);

    /**
     * Constructor.
     */
    public DefaultSPAttributeMapper() { 
        logger.debug("DefaultSPAttributeMapper.constructor");
    }
        ...
        try {
            ...
            if (configMap == null || configMap.isEmpty()) {
                if (logger.isDebugEnabled()) {
                    logger.debug("DefaultSPAttributeMapper.getAttr: Configuration map is not defined.");
                }
                return emptyMap();
            }
            if (logger.isDebugEnabled()) {
                logger.debug("DefaultSPAttributeMapper.getAttr: hosted SP attribute map = " + configMap);
            }
            ...
            logger.error("DefaultSPAccountMapper.getAttributes:MetaException", se);
