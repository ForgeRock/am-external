...
public class SPSingleLogout {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SPSingleLogout.class);
    static SAML2MetaManager sm = null;
    static AssertionFactory af = AssertionFactory.getInstance();
    private static Logger debug = LoggerFactory.getLogger(SPSingleLogout.class);
    ...
    public static void initiateLogoutRequest(
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("SPSingleLogout:initiateLogoutRequest");
        ...
        try {
            ...
            debug.debug("spEntityID : " + spEntityID);
            ...
            if (infoKeyString == null) {
                ...
                // relay state if any
                debug.warn("SPSingleLogout.initiateLogoutRequest : Unable to get infoKeyString from session.");
    ...
    public static Map<String, String> processLogoutResponse(
        ...
        if (logoutRes == null) {
            if (debug.isDebugEnabled()) {
                debug.debug("SSingleLogout:processLogoutResponse: logoutRes " +
                        "is null");
            }
            return null;
        }
        ...
        if (logoutReq == null && isFailoverEnabled()) { //check the samlFailover cache instead
            try {
        ...
        try {
            ...
            debug.debug("SPSingleLogout.processLogoutResponse() : Unable to invalidate session: " + se.getMessage());
        }
        if (!SPCache.isFedlet) {
    ...
    public static void processLogoutRequest(
        ...
        if (debug.isDebugEnabled()) {
            debug.debug(method + "samlRequest : " + samlRequest);
            debug.debug(method + "relayState : " + relayState);
        }
        String rmethod = request.getMethod();
        String binding = SAML2Constants.HTTP_REDIRECT;
        ...
        if (logoutReq == null) {
            if (debug.isDebugEnabled()) {
                debug.debug("SPSingleLogout:processLogoutRequest: logoutReq " +
                        "is null");
            }
            return;
        }
        ...
        if (partners != null && !partners.isEmpty()) {
            ...
            IDPProxyUtil.sendIDPInitProxyLogoutRequest(request, response, out,
                    logoutRespon, location, spEntityID, idpEntityID, binding, realm, relayState);
        } else {
    ...
    public static LogoutResponse processLogoutRequest(
        ...
        try {
            do {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug(method + "infokey=" + infoKeyString);
                }
                if (SPCache.isFedlet) {
                    // verify request
                    if(!isVerified &&
                ...
                List list = (List)SPCache.fedSessionListsByNameIDInfoKey
                        .get(infoKeyString);
                if (debug.isDebugEnabled()) {
                    debug.debug(method + "SPFedsessions=" + list);
                }
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug(method + "isLBReq = " + isLBReq +
                            ", foundPeer = " + foundPeer);
                }
                ...
                JAXBElement<BaseConfigType> spConfig = SAML2Utils.getSAML2MetaManager()
