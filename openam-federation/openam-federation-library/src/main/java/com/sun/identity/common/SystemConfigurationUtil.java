...
public final class SystemConfigurationUtil implements ConfigurationListener {
    ...
    private static Logger debug = LoggerFactory.getLogger(SystemConfigurationUtil.class);
    ...
    public static Set<String> getCookieDomainsForRequest(HttpServletRequest request) {
        ...
        return CookieUtils.getMatchingCookieDomains(request.getServerName(), cookieDomains);
    }
    ...
    throws SystemConfigurationException {
        ...
        try {
            ...
            debug.error("SystemConfigurationUtil.getServiceURL:", cex);
        }
     ...
     * @throws SystemConfigurationException if unable to get the server list.
     */
    public static List<String> getSiteList() throws SystemConfigurationException {
        if (!platformNamingInitialized) {
            initPlatformNaming();
        }
        return siteList;
            ...
            for (String serverEntry : servers) {
                try {
                    ...
                    if (debug.isDebugEnabled()) {
                        debug.debug("SystemConfigUtil.storeServerAndSiteList: " +
                            "adding server " + entry.getId() + ": " + entry.getUrl());
                    }
                } catch (IllegalArgumentException ex) {
                    debug.error("SystemConfigurationUtil.storeServerAndSiteList: " +
    ...
    public void configChanged(ConfigurationActionEvent event) {
        if (debug.isDebugEnabled()) {
            debug.debug("SystemConfigurationUtil.configChanged: " +
        ...
        try {
            ...
            debug.error("SystemConfigurationUtil.configChanged: ", ex);
        }
    }
    ...
    private static synchronized void update() throws ConfigurationException {
        ...
        if ((avPairs == null) || avPairs.isEmpty()) {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("SystemConfigUtil.update: " +
                    "servers=" + servers);
                debug.debug("SystemConfigUtil.update: " +
                    "sites=" + sites);
            }
    ...
    private static synchronized void initPlatformNaming() {
        ...
        try {
            ...
            debug.error("SystemConfigurationUtil.initPlatformNaming: unable to get " +
        ...
        try {
            ...
            debug.error("SystemConfigurationUtil.initPlatformNaming: ", ex);
        }
    }
