...
public class Subject {
    ...
    */     
    public Subject(NameIdentifier nameIdentifier)  throws SAMLException { 
        if (nameIdentifier == null) {
            if (logger.isDebugEnabled()) {
                logger.debug("Subject:  null NameIdentifier "
                    + "specified");
            }
    ...
    public Subject(org.w3c.dom.Element subjectElement)  throws SAMLException {
        ...
        if (eltName == null)  {
            if (logger.isDebugEnabled()) {
                logger.debug("Subject: local name missing");
            }
            throw new SAMLRequesterException(SAMLUtilsCommon.bundle.getString
                ("nullInput")) ;   
        }
        if (!(eltName.equals("Subject")))  {
            if (logger.isDebugEnabled()) {
                logger.debug("Subject: invalid root element");
            }
            throw new SAMLRequesterException(SAMLUtilsCommon.bundle.getString(
                "invalidElement")) ;   
        }
        ...
        if (length == 0 ) {
            if (logger.isDebugEnabled()) {
                logger.debug("Subject: No sub elements found");
            }
            throw new SAMLRequesterException(SAMLUtilsCommon.bundle.getString(
                "emptyElement")) ;   
        }
        ...
        for (int n=0; n < length; n++) {
            ...
            if (childName.equals("NameIdentifier"))  {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Subject: Invalid element "
                        + "encountered.");
                }
                throw new SAMLRequesterException(
        ...
        if (elementCount > 2 ) {
            if (logger.isDebugEnabled()) {
                logger.debug("Subject: more than allowed " +
                    "elements passed");
            }
            throw new SAMLRequesterException(SAMLUtilsCommon.bundle.getString(
                "moreElement")) ;   
    ...
    */
    public boolean removeSubjectConfirmation()  {
        if (_nameIdentifier == null) {
            if (logger.isDebugEnabled())  {
                logger.debug("Subject:At least one of " + 
