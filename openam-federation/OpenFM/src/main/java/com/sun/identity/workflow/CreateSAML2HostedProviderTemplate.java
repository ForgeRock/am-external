...
public class CreateSAML2HostedProviderTemplate {
    ...
    private static void buildIDPConfigTemplate(StringBuffer buff, String idpAlias, String url, Map mapParams)  {
        ...
        buff.append("    <IDPSSOConfig metaAlias=\"").append(idpAlias).append("\">\n")
                .append("        <Attribute name=\"").append(SAML2Constants.ENTITY_DESCRIPTION).append("\">\n")
                .append("            <Value></Value>\n")
                .append("        </Attribute>\n");
        if (StringUtils.isNotEmpty(idpSCertAlias)) {
            buff
                    .append("        <Attribute name=\"").append(SAML2Constants.SIGNING_CERT_ALIAS).append("\">\n")
                    .append("            <Value>").append(idpSCertAlias).append("</Value>\n")
                    .append("        </Attribute>\n");
        }
        if (StringUtils.isNotEmpty(idpECertAlias)) {
            buff
