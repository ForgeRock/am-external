...
public class LogProvider implements com.sun.identity.plugin.log.Logger {
    ...
    private static org.slf4j.Logger debug = LoggerFactory.getLogger(LogProvider.class);
        ...
        SSOToken authSSOToken)
    {
        ...
        if (session != null) {
            try {
                ...
                debug.debug("Error getting session provider: " , se);
            } catch (SSOException soe) {
                debug.debug("Error creating SSOToken: " , soe);
            }
        }
