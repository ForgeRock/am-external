...
public class DefaultADFSPartnerAccountMapper 
    extends DefaultLibrarySPAccountMapper {
     ...
     private static final Logger logger = LoggerFactory.getLogger(DefaultADFSPartnerAccountMapper.class);
    ...
    public DefaultADFSPartnerAccountMapper() {
        super();
        logger.debug("DefaultADFSPartnerAccountMapper.constructor: ");
    }
        ...
        throws WSFederationException
    {
        ...
        if ( nameID.getFormat().equals(WSFederationConstants.NAMED_CLAIM_TYPES[
            ...
            if ( logger.isDebugEnabled() ) {
                logger.debug(classMethod + "domain is "+upnDomain);
            }
        } else {
            name = nameValue;
        } 
        ...
        if ( logger.isDebugEnabled() ) {
            logger.debug(classMethod + "name is "+name);
        }
    ...
    protected boolean isDynamicalOrIgnoredProfile(String realm) {
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("dynamicalCreationEnabled, attr=" + tmp);
            }
            ...
            logger.error("dynamicalCreationEnabled, unable to get attribute", e);
            return false;
        }
    }
}
