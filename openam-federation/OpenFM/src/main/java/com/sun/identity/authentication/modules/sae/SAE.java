...
public class SAE extends AMLoginModule {
    ...
    static {
	debug = LoggerFactory.getLogger(SAE.class);
    }
    ...
    public int process (Callback[] callbacks, int state) 
        ...
        debug.debug("SAE AuthModule.process...");
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("SAE AuthModule.process+encodedStr="+encodedString);
        }
        ...
        debug.debug("SAE AuthModule.SAML2Utils.getSAEAttrs");
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("SAE AuthModule: realm=" + realm +
        ...
        try {
            ...
            if (debug.isDebugEnabled()) 
                debug.debug("SAE AuthModule.: SAE attrs:"+attrs); 
        } catch (Exception ex) {
        ...
        while(iter.hasNext()) {
           ...
           if(debug.isDebugEnabled()) {
              debug.debug("Session Property set: " + key + "= " + value);
           }
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("SAE AuthModule: auth level = " + authLevelInt);
        }
        setAuthLevel(authLevelInt);
        debug.debug("SAE AuthModule:return SUCCESS");
