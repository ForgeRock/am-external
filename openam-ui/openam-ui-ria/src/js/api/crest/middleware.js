...
const middleware = (promise) => {
    return promise.catch((error) => {
        if (error instanceof CRESTError) {
            ...
            Messages.addMessage({ message: t("config.messages.AppMessages.requestError"), type: Messages.TYPE_DANGER });
        } else if (error instanceof ParseError) {
            Messages.addMessage({ message: t("config.messages.AppMessages.parseError"), type: Messages.TYPE_DANGER });
        }
