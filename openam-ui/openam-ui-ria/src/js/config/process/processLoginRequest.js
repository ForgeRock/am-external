...
const processLoginRequest = (event) => {
    ...
    SessionManager.login(submitContent, (user) => {
        ...
        const hasRESTLoginDialog = Configuration.hasRESTLoginDialog && typeof $.prototype.modal === "function";

        if (hasRESTLoginDialog) {
            $(".modal.in").modal("hide");
            // There are some cases, when user is presented with login modal panel,
            // rather than a normal login view. Configuration.hasRESTLoginDialog property is used to
            // indicate such cases. It should be deleted afterwards for correct
            // display of the login view later
            delete Configuration.hasRESTLoginDialog;
            Router.checkRole(Router.currentRoute);
        } else if (Configuration.gotoFragment &&
