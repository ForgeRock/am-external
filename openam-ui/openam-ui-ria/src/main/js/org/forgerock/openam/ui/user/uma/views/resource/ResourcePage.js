...
define([
    ...
    "templates/user/uma/backgrid/cell/SelectizeCell",
    "templates/user/uma/backgrid/cell/RevokeCell",

    // jquery dependencies
    "selectize"
], ($, _, AbstractView, Backbone, Backgrid, BackgridUtils, BootstrapDialog, CommonShare, Constants,
    EventManager, LabelTreeNavigationView, Messages, Router, UIUtils, UMAService, UMAResourceSetWithPolicy,
    SelectizeCell, RevokeCellTemplate) => {
    ...
    function createLabels (labelNames) {
        var creationPromises = _.map(labelNames, (labelName) => {
        ...
        renderWithModel (callback) {
            ...
            RevokeCell = BackgridUtils.TemplateCell.extend({
                ...
                revoke () {
                    ...
                    self.model.get("policy").save().done(() => {
                        self.onModelChange(self.model);
                    });
                },
            ...
            this.renderSelectizeCell().done((SelectizeCell) => {
