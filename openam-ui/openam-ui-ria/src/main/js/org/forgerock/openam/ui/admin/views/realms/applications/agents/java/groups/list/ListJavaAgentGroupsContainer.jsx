...
import { setInstances } from "store/modules/remote/config/realm/applications/agents/java/groups/instances";
import connectWithStore from "components/redux/connectWithStore";
import ListJavaAgentGroups from "./ListJavaAgentGroups";
...
class ListJavaAgentGroupsContainer extends Component {
    constructor () {
        super();
    ...
    handleTableDataChange (pagination) {
        ...
        const additionalParams = {
            fields: ["globalJ2EEAgentConfig/status"],
            pagination
        };
    ...
    handleDelete (items) {
        ...
        showConfirmationBeforeAction({
            message: t("console.applications.agents.java.groups.confirmDeleteSelected", { count: ids.length })
        }, () => {
    ...
    render () {
        const realm = this.props.router.params[0];
        const newHref = Router.getLink(Router.configuration.routes.realmsApplicationsAgentsJavaAgentGroupsNew, [
...
ListJavaAgentGroupsContainer.propTypes = {
...
ListJavaAgentGroupsContainer = connectWithStore(ListJavaAgentGroupsContainer,
    (state) => ({
        groups: values(state.remote.config.realm.applications.agents.java.groups.instances)
    }),
    (dispatch) => ({
        setInstances: bindActionCreators(setInstances, dispatch)
    })
);
ListJavaAgentGroupsContainer = withRouter(ListJavaAgentGroupsContainer);
ListJavaAgentGroupsContainer = withPagination(ListJavaAgentGroupsContainer);

export default ListJavaAgentGroupsContainer;
