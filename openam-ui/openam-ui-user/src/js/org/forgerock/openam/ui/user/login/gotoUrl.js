...
export function setValidated (value) {
    ...
    _.set(Configuration, "globalData.auth.validatedGoto", encodeURIComponent(ifRelativePathInsertContext(value)));
}
...
export function validateParam (unvalidated) {
    ...
    validateGotoService(unvalidated).then((responce) => {
        if (isNotDefaultPath(responce.successURL)) {
            deferred.resolve(responce.successURL);
        } else if (isNotDefaultPath(unvalidated)) {
            deferred.reject();
        } else {
            deferred.resolve(responce.successURL);
        }
