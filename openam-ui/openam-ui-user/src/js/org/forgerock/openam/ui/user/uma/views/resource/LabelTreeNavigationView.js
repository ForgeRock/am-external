...
const LabelTreeNavigationView = TreeNavigation.extend({
    template: "user/uma/views/resource/LabelTreeNavigationTemplate",
    partials: {
        "uma/_NestedList": "uma/_NestedList"
    },
    findActiveNavItem (fragment) {
        const myLabelsRoute = Router.configuration.routes.umaResourcesMyLabelsResource;

        const isCurrentRouteForResource = Router.currentRoute === myLabelsRoute;

        const subFragment = isCurrentRouteForResource ? _.initial(fragment.split("/")).join("/") : fragment;

        const anchor = this.$el.find(`.sidenav ol > li > a[href='#${subFragment}']`);
    ...
    addToParent (collection, label) {
        if (label.name.indexOf("/") === -1) {
            ...
            const shift = label.name.split("/");

            const parentName = shift.shift();
