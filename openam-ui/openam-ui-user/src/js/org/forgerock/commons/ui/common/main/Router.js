...
Router.init = function (routes) {
    ...
    const CustomRouter = Backbone.Router.extend({
        ...
        routeCallback (route) {
             ...
             */
            const args = Router.applyDefaultParameters(
                route,
                Router.extractParameters(route, URIUtils.getCurrentFragment())
            );
...
Router.getLink = function (route, rawParameters) {
    ...
    let i = 0;
    let pattern = _.isRegExp(route.url) ? route.pattern : route.url.replace(/:[A-Za-z@.]+/, "?");
