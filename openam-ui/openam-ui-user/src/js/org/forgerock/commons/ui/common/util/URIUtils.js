...
URIUtils.getCurrentCompositeQueryString = function () {
    const urlQueryString = URIUtils.getCurrentQueryString();

    const fragmentQueryString = URIUtils.getCurrentFragmentQueryString();
 ...
 * @see https://bugzilla.mozilla.org/show_bug.cgi?id=135309
...
URIUtils.parseQueryString = function (queryString) {
    if (queryString) {
        return _.fromPairs(_.map(queryString.split("&"), (pair) => {
