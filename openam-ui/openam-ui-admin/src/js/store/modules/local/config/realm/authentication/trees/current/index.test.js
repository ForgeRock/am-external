...
import injector from "inject-loader!./index";
...
describe("store/modules/local/config/realm/authentication/trees/current/index", () => {
    ...
    const PAGE_TYPE = PAGE_NODE_TYPE;
    ...
    describe("actions", () => {
        describe("#moveNodeToTree", () => {
            it("creates an action", () => {
                expect(moveNodeToTree("node-id")).eql({
        ...
        describe("#moveNodeToPage", () => {
            it("creates an action", () => {
                expect(moveNodeToPage({ nodeId: "node-id", pageId: "page-id" })).eql({
        ...
        describe("#setDisplayName", () => {
            it("creates an action", () => {
                expect(setDisplayName({ displayName: "Node name", id: "node-id" })).eql({
    ...
    describe("reducer", () => {
        ...
        const nodeProperties = (id, type = "MyNode", extraProperties = {}) => assign({
            _id: id,
            _type: { _id: type }
        }, extraProperties);
        ...
        const state = {
            nodes: {
                dimensions: {
                    node1: assign(dimensions(), { id: "node1" }),
                    node2: assign(dimensions(), { id: "node1" }),
                    node3: assign(dimensions(), { id: "node1" }),
                    pageNode: assign(dimensions(), { id: "pageNode" })
                },
                pages: {
                    childnodes: {
                        childNode1: "pageNode",
        ...
        describe("#moveNodeToPage", () => {
            describe("moving a node that is in the same page", () => {
                it("leaves the state unchanged", () => {
                    const action = moveNodeToPage({ nodeId: "childNode1", pageId: "pageNode" });
                    expect(reducer(state, action)).eql(state);
                });
            });
            describe("moving a node that is in another page", () => {
                beforeEach(() => {
                    ...
                    const action = moveNodeToPage({ nodeId: "childNode1", pageId: "pageNode2" });
                ...
                it("leaves the other node properties unchanged", () => {
                    expect(omit(newState.nodes.properties, ["pageNode", "pageNode2"]))
                        .eql(omit(state.nodes.properties, ["pageNode", "pageNode2"]));
                });
            });
            describe("moving a node that is in a tree", () => {
                beforeEach(() => {
                    const action = moveNodeToPage({ nodeId: "node1", pageId: "pageNode" });
                    newState = reducer(state, action);
                });
                it("removes the node from the tree", () => {
                    expect(newState.tree.nodes).not.have.property("node1");
                });
                it("removes the node's dimensions", () => {
                    expect(newState.nodes.dimensions).not.have.property("node1");
                });
        ...
        describe("#moveNodeToTree", () => {
            describe("moving a node that is in a page", () => {
                beforeEach(() => {
                    newState = reducer(state, moveNodeToTree("childNode1"));
                });
            ...
            describe("moving a node that is not in a page", () => {
                it("leaves the state unchanged", () => {
                    expect(reducer(state, moveNodeToTree("node1"))).eql(state);
                });
            });
        });
        describe("#removeNode", () => {
            describe("when the node is a top-level node in the tree", () => {
                beforeEach(() => {
                    newState = reducer(state, removeNode("node1"));
                });
                ...
                it("removes the node properties", () => {
                    expect(newState.nodes.properties).eql(omit(state.nodes.properties, "node1"));
                });
                it("removes the node dimensions", () => {
                    expect(newState.nodes.dimensions).eql(omit(state.nodes.dimensions, "node1"));
                });
            ...
            describe("when the node is in a page", () => {
                beforeEach(() => {
                    newState = reducer(state, removeNode("childNode1"));
                });
                it("removes the node from the page", () => {
                    expect(newState.nodes.properties.pageNode.nodes).eql([{
                        _id: "childNode2",
                ...
                it("removes the node's properties", () => {
                    expect(newState.nodes.properties).eql(assign(omit(state.nodes.properties, "childNode1"), {
                        pageNode: nodeProperties("pageNode", PAGE_TYPE, {
                            nodes: [{
                                _id: "childNode2",
                                nodeType: "MyNode",
                ...
                it("removes the node from the childnodes object", () => {
                    expect(newState.nodes.pages.childnodes).eql(omit(state.nodes.pages.childnodes, "childNode1"));
                });
                it("leaves the rest of the state unchanged", () => {
                    expect(newState.nodes.dimensions).eql(state.nodes.dimensions);
            ...
            describe("when the node is a page", () => {
                beforeEach(() => {
                    newState = reducer(state, removeNode("pageNode"));
                });
                it("removes the node's properties", () => {
                ...
                it("removes it from the dimensions", () => {
                    expect(newState.nodes.dimensions).to.not.have.property("pageNode");
                });
        ...
        describe("#removeCurrentTree", () => {
            it("replaces the state with a blank copy", () => {
                expect(reducer(state, removeCurrentTree())).eql({
                    nodes: {
                        dimensions: {},
                        pages: {
                            childnodes: {},
        ...
        describe("#setDisplayName", () => {
            describe("when the node is in a page", () => {
                beforeEach(() => {
                    newState = reducer(state, setDisplayName({
                ...
                it("makes no other change", () => {
                    expect(newState.tree).eql(state.tree);
                    expect(omit(newState.nodes, "properties")).eql(omit(state.nodes, "properties"));
                    expect(omit(newState.nodes.properties, "pageNode"))
                        .eql(omit(state.nodes.properties, "pageNode"));
                });
            });
            describe("when the node is in the tree", () => {
                ...
                it("makes no change to other nodes in the tree", () => {
                    expect(omit(newState.tree.nodes, "node2")).eql(omit(state.tree.nodes, "node2"));
                });
                it("makes no other change", () => {
