...
describe("store/modules/local/config/global/secretStores/instances/current/mappings/instances", () => {
    const instanceOne = {
        _id: "one",
        purpose: "purposeOne",
        alias: "aliasOne"
        ...
        describe("#addOrUpdate", () => {
            it("creates an action", () => {
                expect(addOrUpdate(instanceOne)).eql({
                    payload: instanceOne,
                    type: "local/config/global/secretStores/instances/current/mappings/instances/ADD_OR_UPDATE"
                });
            });
        });
        ...
        describe("#remove", () => {
            it("creates an action", () => {
                expect(remove("one")).eql({
                    type: "local/config/global/secretStores/instances/current/mappings/instances/REMOVE",
                    payload: "one"
                });
            });
        });
        ...
        describe("#set", () => {
            it("creates an action", () => {
                expect(set([instanceOne])).eql({
                    payload: [instanceOne],
                    type: "local/config/global/secretStores/instances/current/mappings/instances/SET"
                });
            });
        });
    });
