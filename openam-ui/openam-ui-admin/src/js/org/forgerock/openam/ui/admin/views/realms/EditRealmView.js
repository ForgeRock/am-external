...
import EditRealmNameValidationErrorTemplate from "templates/admin/views/realms/EditRealmNameValidationErrorTemplate";
import EditRealmTemplate from "templates/admin/views/realms/EditRealmTemplate";
...
function validateRealmName () {
    ...
    if (realmName.length > 0) {
        if (checkPattern(realmName)) {
            alert = EditRealmNameValidationErrorTemplate();
        } else {
            valid = true;
        }
    }
...
const EditRealmView = AbstractView.extend({
    ...
    render (args) {
        ...
        if (this.data.newEntity) {
            realmPromise = realmSchema();
            authenticationPromise = GlobalAuthenticationService.authentication.schema();
        } else {
            realmPromise = getRealm(this.data.realmPath);
            authenticationPromise = RealmAuthenticationService.authentication.get(this.data.realmPath);
        }

        this.parentRender(function () {
            Promise.all([realmPromise, getAllRealms(), authenticationPromise]).then((response) => {
                const data = response[0];
                const allRealmPaths = _.map(response[1].result, "path");
                if (this.data.newEntity) {
    ...
    handleSave (event) {
        ...
        const savePromise = this.data.newEntity ? createRealm(values) : updateRealm(values);
        ...
        savePromise.then((realm) => {
            ...
            Promise.all([fqdnPromise, statelessPromise]).finally(() => {
                if (this.data.newEntity) {
                    this.data.newEntity = false;
                    Router.routeTo(Router.configuration.routes.realmDefault, {
    ...
    onDeleteClick (event) {
        ...
        showDeleteDialog({
            names: [this.data.realmName],
            objectName: "realm",
            onConfirm: () => this.deleteRealm()
        });
    },
    ...
    deleteRealm () {
        ...
        return removeRealm(this.realmPath).then(() => {
            updateDefaultServerAdvancedFqdnMap(
                this.realmPath,
                [],
                this.originalDnsAliases
