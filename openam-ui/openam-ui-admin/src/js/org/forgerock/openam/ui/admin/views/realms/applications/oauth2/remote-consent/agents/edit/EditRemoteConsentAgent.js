...
import EditRemoteConsentAgentTemplate from
    "templates/admin/views/realms/applications/oauth2/remote-consent/agents/edit/EditRemoteConsentAgentTemplate";
...
const addGroupSelectionToSchema = (schema, groups) => {
    ...
    if (agentgroupProperty) {
        ...
        console.error("[EditRemoteConsentAgent] Unable to add available Remote Consent Agent Groups to " +
            "\"properties.agentgroup\" property.");
    }
};

class EditRemoteConsentAgent extends AbstractView {
    constructor () {
        super();

        this.template = EditRemoteConsentAgentTemplate;
        this.events = {
    ...
    getGroupValues (id, realm) {
        return id ? getGroup(realm, REMOTE_CONSENT_AGENT, id) : $.Deferred().resolve({});
    }
    ...
    render ([realm, id]) {
        ...
        Promise.all([
            ...
            getClient(realm, REMOTE_CONSENT_AGENT, id),
            getAllGroups(realm, REMOTE_CONSENT_AGENT)
    ...
    onSave () {
        ...
        update(this.realm, REMOTE_CONSENT_AGENT, valuesWithoutNullPasswords.raw, this.data.id).then(() => {
...
export default EditRemoteConsentAgent;
