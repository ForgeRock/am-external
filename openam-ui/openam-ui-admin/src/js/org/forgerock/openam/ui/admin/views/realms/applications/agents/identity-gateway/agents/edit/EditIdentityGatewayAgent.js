...
import EditIdentityGatewayAgentTemplate from
    "templates/admin/views/realms/applications/agents/identity-gateway/agents/edit/EditIdentityGatewayAgentTemplate";
...
const addGroupSelectionToSchema = (schema, groups) => {
    ...
    if (agentgroupProperty) {
        ...
        console.error("[EditIdentityGatewayAgent] Unable to add available Identity Gateway Agent Groups to " +
            "\"properties.agentgroup\" property.");
    }
};

class EditIdentityGatewayAgent extends AbstractView {
    constructor () {
        super();

        this.template = EditIdentityGatewayAgentTemplate;
        this.events = {
    ...
    getGroupValues (id, realm) {
        return id ? getGroup(realm, IDENTITY_GATEWAY_AGENT, id) : $.Deferred().resolve({});
    }
    ...
    render ([realm, id]) {
        ...
        Promise.all([
            getSchema(realm, IDENTITY_GATEWAY_AGENT),
            getClient(realm, IDENTITY_GATEWAY_AGENT, id),
            getAllGroups(realm, IDENTITY_GATEWAY_AGENT)
    ...
    onSave () {
        ...
        update(this.realm, IDENTITY_GATEWAY_AGENT, valuesWithoutNullPasswords.raw, this.data.id).then(() => {
    ...
    onDelete () {
        showDeleteDialog({
            ...
            onConfirm: async () => {
                try {
                    await remove(this.realm, IDENTITY_GATEWAY_AGENT, [this.data.id]);
                    Router.routeTo(Router.configuration.routes.realmsApplicationsAgentsIdentityGateway, {
...
export default EditIdentityGatewayAgent;
