...
describe("org/forgerock/openam/ui/common/models/JSONValues", () => {
    ...
    describe("#addValueForPath", () => {
        it("adds values for the provided path", () => {
            const originalValues = new JSONValues({
                one: {
                    two: {
                        three: "value1"
                    }
            ...
            let newValues = originalValues.addValueForPath("one.two.three", "changedValue1");
            expect(newValues.raw.one.two.three).eq("changedValue1");

            newValues = originalValues.addValueForPath("four", "changedValue2");
            expect(newValues.raw.four).eq("changedValue2");
        });

        context("when the provided path contains dot notated key", () => {
            context("when the dot notated key is the last key in the path", () => {
                const originalObject = new JSONValues({
                    prop: {
                        dot: {
                            notated: {
                                key: "value1"
                        ...
                        "dot.notated.key": "value2"
                    }
                });

                it("updates the correct value", () => {
                    const newValues = originalObject.addValueForPath(["prop", "dot.notated.key"], "changedValue");
                    expect(newValues.raw.prop.dot.notated.key).eq("value1");
                    expect(newValues.raw.prop["dot.notated.key"]).eq("changedValue");
                });
            });

            context("when the dot notated key is the only key in the path", () => {
                const originalObject = new JSONValues({
                    dot: {
                        notated: {
                            key: "value1"
                ...
                it("updates the correct value", () => {
                    const newValues = originalObject.addValueForPath(["dot.notated.key"], "changedValue");
                    expect(newValues.raw.dot.notated.key).eq("value1");
                    expect(newValues.raw["dot.notated.key"]).eq("changedValue");
                });
            });

            context("when the path contains multiple dot notated keys", () => {
                const originalObject = new JSONValues({
                    first: {
                        dot: {
                            notated: {
                    ...
                    "first.dot.notated.key": {
                        ...
                        "second.dot.notated.key": "value3"
                    }
                });

                it("updates the correct value", () => {
                    const newValues = originalObject.addValueForPath(
                        ["first.dot.notated.key", "second.dot.notated.key"],
                        "changedValue");
                    expect(newValues.raw.first.dot.notated.key).eq("value1");
                    expect(newValues.raw["first.dot.notated.key"].second.dot.notated.key).eq("value2");
                    expect(newValues.raw["first.dot.notated.key"]["second.dot.notated.key"]).eq("changedValue");
                });
            });
            ...
            context("when the path doesn't exist", () => {
                it("adds the values", () => {
                    ...
                    let newValues = originalValues.addValueForPath("prop1.prop2", "firstValue");
                    expect(newValues.raw.prop1.prop2).eq("firstValue");

                    newValues = originalValues.addValueForPath(
                        ["dot.notated.key", "prop1", "prop2"],
                        "secondValue");
                    expect(newValues.raw["dot.notated.key"].prop1.prop2).eq("secondValue");
                    expect(newValues.raw.dot).to.be.undefined;

                    newValues = originalValues.addValueForPath(
                        ["first.dot.notated.key", "prop", "second.dot.notated.key"],
                        "thirdValue");
                    expect(newValues.raw["first.dot.notated.key"].prop["second.dot.notated.key"]).eq("thirdValue");
                    expect(newValues.raw.first).to.be.undefined;
                    expect(newValues.raw["first.dot.notated.key"].prop.second).to.be.undefined;
                });
            });
        });
    });
    describe("#addInheritance", () => {
        ...
        it("returns the unchanged values when input is undefined", () => {
            let result = jsonValues.addInheritance(undefined);
            expect(result.raw.propertyKey.inherited).to.be.undefined;
        });
    });
