...
import LegacyListItemView from "./LegacyListItemView";
import ManageRulesTemplate from "templates/admin/views/realms/authorization/policies/conditions/ManageRulesTemplate";
import OperatorRulesView from "./OperatorRulesView";

export default AbstractView.extend({
    template: ManageRulesTemplate,
    noBaseTemplate: true,
    events: {
    ...
    buildList () {
        const self = this;
        let newRule = null;
        const operators = _.map(this.data.operators, "title");
        let properties = null;
    ...
    initSorting () {
        ...
        this.$el.find("ol#dropbox").sortable({
            ...
            isValidTarget (item, container) {
                const notValid = container.items.length > 0 &&
            ...
            serialize ($parent, $children, parentIsContainer) {
                const result = $.extend({}, $parent.data().itemData);
    ...
    editStart (item) {
        ...
        const self = this;
        const editRuleView = self.getNewRule();
        ...
        const disabledConditions = this.$el.find(".rule, .operator").not(".editing");
    ...
    editStop (item) {
        ...
        const editRuleView = $.extend(false, item, this.getNewRule());
    ...
    addOperator (e) {
        ...
        const operatorRules = new OperatorRulesView();
    ...
    addCondition (e) {
        ...
        const editRuleView = this.getNewRule();
        const self = this;
    ...
    setFocus (e) {
        e.stopPropagation();
        const target = $(e.target).is("select") || $(e.target).is("input") ? e.target : e.currentTarget;
        $(target).focus();
    },
    ...
    save () {
        ...
        const rules = this.$el.find("ol#dropbox").sortable("serialize").get();
        const operatorData = this.$el.find(`#operator${this.idPrefix}0`).data().itemData;
    ...
    identifyDroppableLogical () {
        const rootLogical = this.$el.find(`#operator${this.idPrefix}0`);
        let nestedItems;
        let nestedLogicals;
        let nestedLogicalsLength;
        let nestedRules;
        let logical;
        let canHaveMultiple;
        let notIsEmpty;
