...
class EditCircleOfTrust extends AbstractView {
    ...
    render ([realm, id]) {
        ...
        Promise.all([
            ...
            const modifiedSchema = addEntityProviders(schema, saml2Entities.result, wsFedEntities.result);
            return {
    ...
    onDelete () {
        showDeleteDialog({
            ...
            onConfirm: async () => {
                await remove(this.realm, [this.data.id]);
                Router.routeTo(Router.configuration.routes.realmsApplicationsFederationCirclesOfTrust, {
