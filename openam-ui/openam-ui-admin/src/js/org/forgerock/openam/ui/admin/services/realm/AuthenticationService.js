...
AuthenticationService.authentication = {
    ...
    chains: {
        ...
        remove (realm, ids) {
            const promises = ids.map((id) => AuthenticationService.serviceCall({
                url: fetchUrl(`/realm-config/authentication/chains/${id}`, { realm }),
                headers: { "Accept-API-Version": "protocol=1.0,resource=1.0" },
    ...
    modules: {
        ...
        exists (realm, name) {
            ...
            const query = `_queryFilter=${encodeURIComponent(`_id eq "${name}"`)}&_fields=_id`;
            const request = AuthenticationService.serviceCall({
                url: fetchUrl(
        ...
        remove (realm, id, type) {
            return AuthenticationService.serviceCall({
                url: fetchUrl(`/realm-config/authentication/modules/${type}/${id}`, { realm }),
                headers: { "Accept-API-Version": "protocol=1.0,resource=1.0" },
                suppressSpinner: true,
                type: "DELETE"
            });
