...
import { bind, each, extend, has, isFunction, noop } from "lodash";
import $ from "jquery";
...
export default Backbone.View.extend({
    ...
    parentRender (callback) {
        ...
        getTheme().then((theme) => {
            ...
            if (needsNewBaseTemplate()) {
                ...
                if (isFunction(this.baseTemplate)) {
    ...
    loadTemplate () {
        ...
        each(this.partials, (partial, name) => {
    ...
    loadThemedTemplate (path) {
        return import(`templates/${path}.html`).then(unwrapDefaultExport);
    },
