...
class SetFailureDetailsNodeTest {

    @Mock
    private LocalizedMessageProvider localizedMessageProvider;
    @Mock
    private SetFailureDetailsNode.Config config;
    @Mock
    private Realm realm;
    private UUID uuid = UUID.randomUUID();

    private SetFailureDetailsNode node;

    @BeforeEach
    void setup() {
        node = new SetFailureDetailsNode(uuid, config, realm, (realm) -> localizedMessageProvider);
    }

    @Test
    void testGivenNoLocalizationsReturnsSessionHookWithNoMessage() {
        // given
        given(localizedMessageProvider.getLocalizedMessage(null, SetFailureDetailsNode.class, Map.of(), ""))
                .willReturn(null);
        ...
        assertThat(sessionHook.get(TreeHook.SESSION_HOOK_CLASS_KEY).asString())
                .isEqualTo(FailureDetailsTreeHook.class.getName());
        ...
        assertThat(sessionHook.get(TreeHook.NODE_TYPE_KEY).asString())
                .isEqualTo(SetFailureDetailsNode.class.getSimpleName());
