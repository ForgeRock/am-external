...
public class SocialOAuth2Helper {
    ...
    private OpenIdResolverService openIdResolverService;

    /**
     * Constructor.
     *
     * @param handler the handler for the OAuthClient
     * @param openIdResolverService the resolver service for the OAuthClient
     */
    @Inject
    public SocialOAuth2Helper(@Named("SocialAuthClientHandler") Handler handler,
            @Named(OAuth2Constants.Custom.JWK_RESOLVER) OpenIdResolverService openIdResolverService) {
        this.handler = handler;
        this.openIdResolverService = openIdResolverService;
    }
        ...
        try {
            ...
            if (OpenIDConnectClient.class.isAssignableFrom(oauthClient)) {
                instance = new OpenIDConnectClient(handler, (OpenIDConnectClientConfiguration) config, getClock(),
                        getSecureRandom(), openIdResolverService, new OpenIdResolverServiceConfiguratorImpl());
            } else {
