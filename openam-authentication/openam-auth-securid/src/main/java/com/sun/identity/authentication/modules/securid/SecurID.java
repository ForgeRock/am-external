...
public class SecurID extends AMLoginModule {
    ...
    private static Logger debug = null;
    ...
    static {
        if (debug == null) {
            debug = LoggerFactory.getLogger(SecurID.class);
        }
    }
    ...
    public void init(Subject subject, Map sharedState, Map options) {
        ...
        try {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("SecurID resbundle locale="+getLoginLocale());
            }
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("SecurID:init:subject = " + subject +
            ...
            if (debug.isDebugEnabled()) {
                debug.debug ("SecurID:init:configpath = " +
    ...
    {
        if (debug.isDebugEnabled()) {
            debug.debug("SecurID:setDynamic: isPswd=" + isPswd +
                ", state=" + state); 
        }
        ...
        throws AuthLoginException
    {
        ...
        if (debug.isDebugEnabled()) {
            debug.debug ("SecurID:process: Org = " + wtOrgName +
                "\n\tstate = " + state);
        }
         ...
         */
        if (api == null) {
            debug.debug("SecurID.process:getting Session instance");
            try {
        ...
        if (debug.isDebugEnabled()) {
            debug.debug ("SecurID:process:after configDone: Org = " +
                wtOrgName +
                "\n\tstate = " + state +
        ...
        switch (state) {
            case ISAuthConstants.LOGIN_START:        // initial state (1)
            {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("SecurID.process(): username: " + username);
                }
                    ...
                    switch (authStatus) {
                        ...
                        case AuthSession.NEW_PIN_REQUIRED:  // new PIN mode
                        {
                            ...
                            if (pinState == PinData.CANNOT_CHOOSE_PIN) {
                                ...
                                if (debug.isDebugEnabled()) {
                                    debug.debug("SecurID.process:prompt = " +
                                        msg);
                                }
                            } else {  // huh?
                                debug.error(
                        ...
                        case AuthSession.NEXT_CODE_REQUIRED: // next token mode
                            debug.debug("SecurID.process:NEXT_CODE_REQUIRED");
                            rtnval = ISAuthConstants.LOGIN_NEXT_TOKEN;
                            break;
                        case AuthSession.ACCESS_DENIED:
                            debug.debug("SecurID.process:ACCESS_DENIED");
                        default:
                            debug.debug("SecurID.process:state == default");
            ...
            {
                debug.debug("SecurID.process:LOGIN_CHALLENGE");
                // submit new PIN
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("SecurID.process:state2: token length = " +
                        newPIN.length());
                }
                ...
                try {
                    ...
                    if (debug.isDebugEnabled()) {
                        debug.debug("SecurID.process:session.pin returns " +
                            authStatus);
                    }
                    if (authStatus == AuthSession.PIN_ACCEPTED) {
                        debug.debug("SecurID.process:new pin ACCEPTED");
            ...
            case ISAuthConstants.LOGIN_NEXT_TOKEN:  // next token mode (3)
            {
                ...
                debug.debug("SecurID.process:LOGIN_NEXT_TOKEN");
            ...
            {
                debug.debug("SecurID.process:LOGIN_SYS_GEN_PIN");
                ...
                if (sysgenpin) {
                    ...
                    try {
                        ...
                        authStatus = session.pin(newPin);
                        if (debug.isDebugEnabled()) {
                            debug.debug("SecurID.process:LSGP:" +
            ...
            case ISAuthConstants.LOGIN_NEW_PIN_NEXT_TOKEN: // 5
            {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("LOGIN_NEW_PIN_NEXT_TOKEN:username = " +
                        username);
                }
                ...
                debug.debug("SecurID.process:LNPNT:doing session.check");
                authStatus = AuthSession.ACCESS_DENIED;
                try {
                    authStatus = session.lock(username);
                    if (debug.isDebugEnabled()) {
                        debug.debug("SecurID.process:LNPNT:lock returns " +
                            authStatus);
                    }
                    authStatus = session.check(username, nextToken);
                    if (debug.isDebugEnabled()) {
                        debug.debug("SecurID.process:LNPNT:next returns " +
                            authStatus);
                    }
                } catch (AuthAgentException aaex) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("SecurID.process:LNPNT:ACCESS_OK = " +
                ...
                if (authStatus == AuthSession.ACCESS_OK) {
                    // succeed
                    if (debug.isDebugEnabled()) {
                        debug.debug("SecurID.process:LNPNT:next() =" +
        ...
        if (debug.isDebugEnabled()) {
            debug.debug ("process; after process:" +
                "\n\tstate = " + state +
                "\n\tuserTokenId = " + userTokenId +
                "\n\tusername = " + username +
                "\n\trtnval = " + rtnval);
    ...
    private String getNewPinMsg(PinData pinData) {
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("SecurID.getNewPinMsg:pinState = " + pinState +
                    "\n\tpinMin = " + pinMin + "\n\tpinMax = " + pinMax);
        }
    ...
    public java.security.Principal getPrincipal() {
        if (userPrincipal != null) {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("getPrincipal: returning null");
            }
            return null;
        }
    }
