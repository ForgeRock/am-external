...
/**
 * Tests for {@link AuthModuleGlobalScriptsProvider}.
 */
@RunWith(MockitoJUnitRunner.class)
public class AuthModuleGlobalScriptsProviderTest {

    @InjectMocks
    private AuthModuleGlobalScriptsProvider provider;

    @Test
    public void shouldReturnAuthModuleGlobalScripts() throws ScriptException {
        // When
        List<Script> actual = provider.get();

        // Then
        assertThat(actual.size()).isEqualTo(2);
        assertScriptConfiguration(actual.get(0), AUTH_MODULE_SERVER_SIDE.getId(),
                AUTH_MODULE_SERVER_SIDE.getContext());
        assertScriptConfiguration(actual.get(1), AUTH_MODULE_CLIENT_SIDE.getId(),
                AUTH_MODULE_CLIENT_SIDE.getContext());
    }
