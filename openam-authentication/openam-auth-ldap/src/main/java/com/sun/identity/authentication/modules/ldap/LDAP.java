...
public class LDAP extends AMLoginModule {
    ...
    private final Logger debug = LoggerFactory.getLogger(LDAP.class);
    ...
    public void init(Subject subject, Map sharedState, Map options) {
        ...
        bundle = amCache.getResBundle(LDAP_RESOURCE_BUNDLE_NAME, locale);
        if (debug.isDebugEnabled()) {
            debug.debug("LDAP resbundle locale=" + locale);
        }
        ...
        try {
            ...
            if (loginScreen.equals(LoginScreen.LOGIN_START)) {
                ...
                if (userPassword == null || userPassword.length() == 0) {
                    if (debug.isDebugEnabled()) {
                        debug.debug("LDAP.process: Password is null/empty");
                    }
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("you are in Password Screen:" + currentState);
                }
                ...
                if (pwdAction == 0) {
                    ...
                    if (debug.isDebugEnabled()) {
                        debug.debug("Password change state :" + newState);
                    }
            ...
            if (ResultCode.NO_SUCH_OBJECT.equals(ex.getResultCode())) {
                if (debug.isDebugEnabled()) {
                    debug.debug("The specified user does not exist.");
                }
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("Inappropriate authentication.");
                }

                throw new AuthLoginException(AM_AUTH, "InappAuth", null);
            } else if (ResultCode.CONSTRAINT_VIOLATION.equals(ex.getResultCode())) {
                if (debug.isDebugEnabled()) {
                    debug.debug("Exceed password retry limit.");
                }

                throw new AuthLoginException(LDAP_RESOURCE_BUNDLE_NAME,
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("Invalid Characters detected");
            }
    ...
    private void createProfile() {
        if (isProfileCreationEnabled && userCreationAttrs.size() > 0) {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("user creation attributes: " +
                        userAttributeValues);
            }
    ...
    private Map getAttributeMap(Map attributeValues) {
        if (debug.isDebugEnabled()) {
            debug.debug("In getAttribute Map: " + attributeValues);
        }
        ...
        while (userIterator.hasNext()) {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("key is : " + key);
                debug.debug("value is : " + newValue);
            }
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("New attr map is : " + newAttrMap);
        }
