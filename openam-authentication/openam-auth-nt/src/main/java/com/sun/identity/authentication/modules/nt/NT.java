...
public class NT extends AMLoginModule {
    ...
    public void init(Subject subject, Map sharedState, Map options) {
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("NT resbundle locale="+locale);
        }
    ...
    static {
        if (debug == null) {
            debug =  org.slf4j.LoggerFactory.getLogger(NT.class);
            debug.debug("NT constructor called");
        }
        String base = SystemProperties.getConfigPath();
    ...
    public int process(Callback[] callbacks, int state) 
        ...
        if (host == null || host.length() == 0) {
            debug.debug ("NT Host cannot be null ");
            throw new AuthLoginException(amAuthNT, "Hosterror", null);
        }
        if (domain == null || domain.length() == 0) {
            debug.debug ("NT Domain cannot be null ");   
        ...
        if (callbacks !=null && callbacks.length == 0) {                
            ...
            if (userName == null || userName.length() == 0) {
                debug.debug ("UserId cannot be null");
                throw new AuthLoginException(amAuthNT, "UserIderror", null);
            }
            if (userPassword == null || userPassword.length() == 0) {
                debug.debug ("Password cannot be null");
        ...
        if (debug.isDebugEnabled()) {
            debug.debug ("userName='"+ userName + "' host='" + host + "'");
        ...
        try {
            ...
            if (out.indexOf ("Usage:") != -1) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("smbclient usage error");
                }
                setFailureID(userName);
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("NT authentication failed" + out);
                }
                setFailureID(userName);
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("smbclient timeout error");
                }
                setFailureID(userName);
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("Exit value of samba client: " + exitValue);
                }
                if (exitValue != 0) {
                       if (getCredentialsFromSharedState && !isUseFirstPassEnabled()) {
                        getCredentialsFromSharedState = false;
           ...
           if (getCredentialsFromSharedState && !isUseFirstPassEnabled()) {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("NT authentication failed" + ex.getMessage());
            }
            setFailureID(userName);
