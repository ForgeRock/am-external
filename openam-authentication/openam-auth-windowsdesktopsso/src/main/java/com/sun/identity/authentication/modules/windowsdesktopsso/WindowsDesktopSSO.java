...
public class WindowsDesktopSSO extends AMLoginModule {
    ...
    private Logger debug = LoggerFactory.getLogger(WindowsDesktopSSO.class);
    ...
    public int process(Callback[] callbacks, int state) 
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("Kerberos token retrieved from SPNEGO token: \n" + 
        ...
        try {
            ...
            if (debug.isDebugEnabled()){
                debug.debug("WindowsDesktopSSO kerberos authentication passed succesfully.");
            }
            result = ISAuthConstants.LOGIN_SUCCEED; 
             ...
             if( e instanceof GSSException) {	 
                 ...
                 if (major == GSSException.CREDENTIALS_EXPIRED) {	 
                         debug.debug("Credential expired. Re-establish credential...");	 
                 serviceLogin();	 
                 try {   
                     authenticateToken(kerberosToken, trustedKerberosRealms);  
                     if (debug.isDebugEnabled()){
                       debug.debug("Authentication succeeded with new cred.");    
                           result = ISAuthConstants.LOGIN_SUCCEED;
                     }
                 } catch (Exception ee) {   
    ...
    private void authenticateToken(final byte[] kerberosToken, final Set<String> trustedRealms) 
        ...
        debug.debug("In authenticationToken ...");
        Subject.doAs(serviceSubject, new PrivilegedExceptionAction(){
            public Object run() throws Exception {
                ...
                if (!context.isEstablished()) {
                    ...
                    if (debug.isDebugEnabled()) {
                        debug.debug("Context established !");
                    }
                    ...
                    if (debug.isDebugEnabled()){
                        debug.debug("WindowsDesktopSSO.authenticateToken:"
    ...
    private byte[] parseToken(byte[] rawToken) {
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("token tag:" + DerValue.printByte(tmpToken.getTag()));
        }
        ...
        if (Arrays.equals(oidArray, spnegoOID)) {
            if (debug.isDebugEnabled()) {
                debug.debug("SPNEGO OID found in the Auth Token");
            }
            ...
            if (tmpToken.getTag() == (byte)0xa0) {
                if (debug.isDebugEnabled()) {
                    debug.debug("DerValue: found init token");
                }
                tmpToken = new DerValue(tmpToken.getData());
                if (tmpToken.getTag() == (byte)0x30) {
                    if (debug.isDebugEnabled()) {
                        debug.debug("DerValue: 0x30 constructed token found");
                    }
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("SPNEGO OID not found in the Auth Token");
            }
            ...
            if (!Arrays.equals(krb5Oid, KERBEROS_V5_OID)) {
                if (debug.isDebugEnabled()) {
                    debug.debug("Kerberos V5 OID not found in the Auth Token");
                }
                token = null;
            } else {
                if (debug.isDebugEnabled()) {
                    debug.debug("Kerberos V5 OID found in the Auth Token");
                }
            }
        }
        return token;
    ...
    private boolean getConfigParams() {
        ...
        if (debug.isDebugEnabled()){
            debug.debug("WindowsDesktopSSO params: \n" + 
        ...
        if (debug.isDebugEnabled()){
            debug.debug("Retrieved config params from cache.");
        }
        return true;
    }
    ...
    private void initWindowsDesktopSSOAuth(Map options) 
        ...
        if (debug.isDebugEnabled()){
            debug.debug("Init WindowsDesktopSSO. This should not happen often.");
        }
        verifyAttributes();
        serviceLogin();
    ...
    private synchronized void serviceLogin() throws AuthLoginException{
        ...
        try {
            ...
            if (debug.isDebugEnabled()){
                debug.debug("Service login succeeded.");
            }
        } catch (Exception e) {
    ...
    private String searchUserAccount(String attributeValue, String organization) 
        ...
        if (debug.isDebugEnabled()) {
            debug.debug(classMethod + " searching for user " + attributeValue
        ...
        try {
            ...
            if (searchResults.getErrorCode() == IdSearchResults.SUCCESS && searchResults != null) {
                ...
                if (!results.isEmpty()) {
                    if (debug.isDebugEnabled()) {
                        debug.debug(classMethod + results.size() + " result(s) obtained");
                    }
                    AMIdentity userDNId = results.iterator().next();
                    if (userDNId != null) {
                        if (debug.isDebugEnabled()) {
                             debug.debug(classMethod + "user = " + userDNId.getUniversalId());
                             debug.debug(classMethod + "attrs =" + userDNId.getAttributes(
                                     getUserAliasList()));
                        }
        ...
        if (debug.isDebugEnabled()) {
                    debug.debug(classMethod + " No results were found !");
        }
        return null;
    }
