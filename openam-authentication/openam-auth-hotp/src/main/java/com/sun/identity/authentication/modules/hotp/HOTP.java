...
public class HOTP extends AMLoginModule {
    ...
    private static final Logger debug = LoggerFactory.getLogger(HOTP.class);
    ...
    public void init(Subject subject, Map sharedState, Map options) {
        ...
        try {
            ...
            debug.warn("HOTP.init: unable to retrieve search attributes", ale);
        }
        
        if (debug.isDebugEnabled()) {
            debug.debug("HOTP.init() : " + "telephone attribute=" + telephoneAttribute
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("HOTP.init() : " + "HOTP resouce bundle locale=" + locale);
        }
        ...
        if (userName == null || userName.isEmpty()) {
            try {
                ...
                Session isess = getLoginState("HOTP").getOldSession();
                if (isess == null) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("HOTP.init() : UserName in SSOToken : " + userName);
                }
    ...
    public int process(Callback[] callbacks, int state) throws AuthLoginException {
        if (skip) {
            debug.debug("Skipping HOTP module");
        ...
        if (state == 1) {
            if(hotpAutoClicking) {
                debug.debug("Auto sending OTP code");
                try {
        ...
        try {    
            if (currentState == START_STATE) {
                ...
                if (callbacks != null && callbacks.length == 2) {
                    ...
                    if (debug.isDebugEnabled()) {
                        debug.debug("HOTP.process() : " + "LOGIN page button index: " + action);
                    }
                    ...
                    if (action == 0) { //Submit HOTP Code
                        ...
                        if (enteredHOTPCode == null || enteredHOTPCode.length() == 0) {
                            if (++attempt >= hotpRetryMaxAttempts) {
                                if (debug.isDebugEnabled()) {
                                    debug.debug("HOTP.process() : " + "invalid HOTP code");
                                }
