...
public class HOTPService {
    ...
    private static final Logger DEBUG = LoggerFactory.getLogger(HOTPService.class);
    ...
    public void sendHOTP() throws AuthLoginException {
        try {
            sentHOTPCode = otpGenerator.generateOTP(getSharedSecret(), getMovingFactor(), codeLength, false, 16);
        ...
        if (sentHOTPCode != null && sentHOTPCode.equals(enteredHOTPCode)) {
            ...
            if (timePassed <= (codeValidityDuration * 60000)) {
                ...
                if (DEBUG.isDebugEnabled()) {
                    DEBUG.debug("HOTP.process() : HOTP code has expired");
                }
                return false;
            }
        } else {
            if (DEBUG.isDebugEnabled()) {
                DEBUG.debug("HOTP.process() : HOTP code is not valid");
            }
            return false;
        }
    }
    ...
    private void sendHOTP(String otpCode, String subject, String message) throws AuthLoginException {
        ...
        try {
            ...
            if (phone != null || mail != null) {
                ...
                if (DEBUG.isDebugEnabled()) {
                    DEBUG.debug("HOTP.sendSMS() : IdRepo: no phone or email found with username : " + userName);
                }
                throw new AuthLoginException("HOTP.sendSMS() : Unable to send OTP code "
    ...
    private String getTelephoneNumber(AMIdentity identity) throws IdRepoException, SSOException {
        ...
        if (DEBUG.isDebugEnabled()) {
            DEBUG.debug("HOTP.sendSMS() : Using phone attribute of " + telephoneAttribute);
        }
        ...
        if (telephoneNumbers != null && !telephoneNumbers.isEmpty()) {
            ...
            if (carrierAttribute!=null && carrierAttribute.trim().length()>0) {
                ...
                if (carriers != null && !carriers.isEmpty()) {
                    ...
                    if (DEBUG.isDebugEnabled()) {
                        DEBUG.debug("HOTP.sendSMS() : Using carrier attribute of " + carrierAttribute);
                    }
                }
            }
            if (DEBUG.isDebugEnabled()) {
                DEBUG.debug("HOTP.sendSMS() : " + "IdRepoException : phone number found " + phone
...
                     */
                if (!phone.contains("@")) {
                    DEBUG.debug("HOTP.sendSMS() : No carrier detected - SMSGateway module will use default of "
                            + "@txt.att.net ");
                }
            }
        } else {
            if (DEBUG.isDebugEnabled()) {
                DEBUG.debug("HOTP.sendSMS() : " + "IdRepoException : no phone number found with username : "
                        + userName);
            }
        }
    ...
    private String getEmailAddress(AMIdentity identity) throws IdRepoException, SSOException {
        ...
        if (DEBUG.isDebugEnabled()) {
            DEBUG.debug("HOTP.sendSMS() : " + "Using email attribute of " + emailAttribute);
        }
        ...
        if (emails != null && !emails.isEmpty()) {
            ...
            if (DEBUG.isDebugEnabled()) {
                DEBUG.debug("HOTP.sendSMS() : IdRepo: email address found " + mail + " with username : " + userName);
            }
        } else {
            if (DEBUG.isDebugEnabled()) {
                DEBUG.debug("HOTP.sendSMS() : IdRepo: no email found with username : " + userName);
            }
        }
