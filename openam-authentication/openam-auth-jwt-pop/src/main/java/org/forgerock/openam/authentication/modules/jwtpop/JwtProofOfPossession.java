    ...
    public void init(Subject subject, Map sharedState, Map options) {
        ...
        if (signingKeyPair.getPrivate() instanceof ECPrivateKey) {
            this.challengeSigningKey = new EcJWK((ECPublicKey) signingKeyPair.getPublic(), (ECPrivateKey)
                    signingKeyPair.getPrivate(), KeyUse.SIG, signingKeyId);
            ...
            this.challengeSigningKey = new RsaJWK(((RSAPublicKey) signingKeyPair.getPublic()),
                    ((RSAPrivateKey) signingKeyPair.getPrivate()), KeyUse.SIG, JwsAlgorithm.RS256.getJwaAlgorithmName(),
                    signingKeyId, null, null, null);
        } else {
        ...
        if (responseEncryptionCipher == EncryptionMethod.XC20_P1305) {
            ...
            if (Security.addProvider(new BouncyCastleProvider()) != -1) {
                DEBUG.message("JwtProofOfPossession: Registered Bouncy Castle JCE Provider for ChaCha20-Poly1305");
            }
        }
    ...
    private int processInternal(final int state, final Callback...callbacks) throws LoginException {
        ...
        switch (state) {
            ...
            case RESPONSE_STATE:
                ...
                if (useTlsSessionBinding && getTlsSessionId() != null) {
                    final byte[] actualTlsSessionId = getTlsSessionId().getBytes(StandardCharsets.UTF_8);
                    final byte[] expectedTlsSessionId = verifiedClaims.getClaim(TLS_SESSION_ID_CLAIM, String.class)
        ...
        if (this.subjectJwkSet == null) {
            ...
            try {
                ...
                if (result.getErrorCode() != IdSearchResults.SUCCESS) {
                    DEBUG.message("JwtProofOfPossession.getSubjectJwkSet(): error result: {}", result.getErrorCode());
                    return new JWKSet();
                }

                final AMIdentity profile = (AMIdentity) getFirstItem(result.getSearchResults());
                if (profile == null) {
                    DEBUG.message("JwtProofOfPossession.getSubjectJwkSet(): no identity profile for user: {}", subject);
                    return new JWKSet();
                }
                final String jwkSetStr = (String) getFirstItem(profile.getAttribute(subjectJwkSetAttribute));
                if (jwkSetStr == null) {
                    DEBUG.message("JwtProofOfPossession.getSubjectJwkSet(): null jwk set for user: {}", subject);
                    return new JWKSet();
                }
