...
final class ChallengeResponseVerifier {
    ...
    private static Key decryptionKey(JWK jwk) {
        Reject.ifTrue(jwk.getUse() != null && !ENC.equals(jwk.getUse()), "decryption key is not for encryption!");
    ...
    private static SigningHandler verificationHandler(JWK jwk) {
        Reject.ifTrue(jwk.getUse() != null && !SIG.equals(jwk.getUse()), "verification key is not for signatures!");
