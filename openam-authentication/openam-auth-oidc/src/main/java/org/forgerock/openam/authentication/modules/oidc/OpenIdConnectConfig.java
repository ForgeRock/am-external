...
class OpenIdConnectConfig extends JwtHandlerConfig {
    private static Logger logger = LoggerFactory.getLogger(OpenIdConnectConfig.class);
    ...
    static final String USE_SUB_CLAIM_IF_NO_MATCH = "useSubClaimIfNoMatch";
    ...
    static final String BUNDLE_KEY_INVALID_SIGNING_ALG = "invalid_signing_alg";
    ...
    static final String BUNDLE_KEY_JWT_TYPE_ERROR = "jwt_type_error";
    ...
    private final boolean useSubClaimIfNoMatch;
    ...
    OpenIdConnectConfig(Map options) {
        ...
        useSubClaimIfNoMatch = CollectionHelper.getBooleanMapAttr(options, USE_SUB_CLAIM_IF_NO_MATCH, false);
    ...
    /**
     * Whether the "sub" claim from the id token should be used as the principal if the account mapper
     * returns no matching local account. If false, authentication should fail if there is no local account.
     *
     * @return true if the sub claim should be used when no local account found.
     */
    boolean isSubClaimUsedIfNoMatch() {
