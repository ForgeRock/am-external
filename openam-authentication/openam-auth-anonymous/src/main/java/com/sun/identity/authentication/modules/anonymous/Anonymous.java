...
public class Anonymous extends AMLoginModule {
    ...
    public void init(Subject subject, Map sharedState, Map options) {
        if (debug == null) {
            debug = org.slf4j.LoggerFactory.getLogger(amAuthAnonymous);
        }
        this.sharedState = sharedState;
        java.util.Locale locale = getLoginLocale();
        bundle = amCache.getResBundle(amAuthAnonymous, locale);
        if (debug.isDebugEnabled()) {
            debug.debug("Anonymous resbundle locale="+locale);
        }
        try {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("isCaseSensitive: "+isCaseSensitive);
            }
        } catch(Exception ex) {
            if (debug.isDebugEnabled()) {
                debug.debug("possible exception is ",ex);
            }
            debug.error(
        ...
        try {
            ...
            if (callbacks !=null && callbacks.length > 0) {
                if (callbacks[0] instanceof NameCallback) {
                    ...
                    if (debug.isDebugEnabled()) {
                        debug.debug("Anonymous:process received NameCallback "
                                + usernameParam);
                    }
                    if (processAnonUser(usernameParam)) {
                        return ISAuthConstants.LOGIN_SUCCEED;
    ...
    private String  sendCallback() throws AuthLoginException {
        ...
        try {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("Callback 0 is.. :" + callbacks[0]);
                debug.debug("Callback 1 is.. :" + callbacks[1]);
            }
    ...
    private boolean processAnonUser(String usernameParam)
        ...
        if (usernameParam == null || usernameParam.length() == 0) {
            ...
            debug.debug("user is in anonymoususer's list");
            debug.debug("UserType: non default Anon User");
            userTokenId = usernameParam;
        } else {
            if (useSharedstate) {
                return false;
