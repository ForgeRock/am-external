...
public class DataStore extends AMLoginModule {
    ...
    private final Logger debug = LoggerFactory.getLogger(DataStore.class);
    ...
    public void init(Subject subject, Map sharedState, Map options) {
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("DataStore resbundle locale=" + locale);
        }
    ...
    public int process(Callback[] callbacks, int state)
        ...
        try {
            ...
            if (currentState == ISAuthConstants.LOGIN_START) {
                ...
                if (userName == null) {
                    debug.debug("DataStore.process: Username is null/empty");
                    throw new UserNamePasswordValidationException("amAuth", "InvalidUP", null);
                }
                if (userPassword == null || userPassword.length() == 0) {
                    debug.debug("DataStore.process: Password is null/empty");
                ...
                if (SystemProperties.getAsBoolean(DATASTORE_USER_ONLY, true)) {
                    success = idrepo.authenticate(IdType.USER, idCallbacks);
                } else {
                    success = idrepo.authenticate(idCallbacks);
                }
                if (success) {
