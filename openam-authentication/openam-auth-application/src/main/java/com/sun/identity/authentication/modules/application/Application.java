...
public class Application extends AMLoginModule {
    ...
    private static Logger debug = LoggerFactory.getLogger(Application.class);
    ...
    public void init(Subject subject, Map sharedState, Map options) {
        try {
            ...
            debug.debug("ApplicationAuth resbundle locale={}", locale);
        } catch (Exception e) {
    ...
    private boolean doFallbackAuth(String userName, String userPassword)
        ...
        debug.debug("doFallbackAuth : User = {}", userName);

        if (userName != null && (userName.length() != 0)) {
            if (authenticateToDatastore(userName, userPassword)) {
                debug.debug("Application.doFallbackAuth: Authenticated to AgentsRepo.");
                if (userTokenId == null) {
