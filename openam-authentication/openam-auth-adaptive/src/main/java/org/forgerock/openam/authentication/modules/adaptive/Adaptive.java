 ...
 *
 * Portions Copyrighted 2013-2017 Nomura Research Institute, Ltd.
 */
...
public class Adaptive extends AMLoginModule {
    ...
    private static Logger debug = LoggerFactory.getLogger(Adaptive.class);
    ...
    public void init(Subject subject, Map sharedState, Map options) {
        ...
        try {
            ...
            debug.warn("{}.init: unable to retrieve search attributes", ADAPTIVE, ale);
        }

        if (debug.isDebugEnabled()) {
            debug.debug("{}.init : resbundle locale={}, user search attributes={}", ADAPTIVE, locale,
    ...
    public int process(Callback[] callbacks, int state)
        ...
        debug.debug("{}: process called with state = {}", ADAPTIVE, state);
        ...
        if (userName == null || userName.length() == 0) {
            ...
            try {
                ...
                Session isess = getLoginState(ADAPTIVE).getOldSession();
                if (isess == null) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.process() : UserName '{}' in SSOToken", ADAPTIVE, userName);
                }
                ...
                debug.debug("{}: amAuthIdentity NULL ", ADAPTIVE);
                throw new AuthLoginException(ADAPTIVE, "noIdentity", null);
            }
        }
        if (debug.isDebugEnabled()) {
            debug.debug("{}: Login Attempt Username = {}", ADAPTIVE, userName);
        }
        ...
        try {
            if (IPRangeCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkIPRange: returns {}", ADAPTIVE,  retVal);
                }
                currentScore += retVal;
            }
            if (IPHistoryCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkIPHistory: returns {}", ADAPTIVE, retVal);
                }
                currentScore += retVal;
            }
            if (knownCookieCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkKnownCookie: returns {}", ADAPTIVE, retVal);
                }
                currentScore += retVal;
            }
            if (timeOfDayCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkTimeDay: returns {}", ADAPTIVE, retVal);
                }
                currentScore += retVal;
            }
            if (timeSinceLastLoginCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkLastLogin: returns {}", ADAPTIVE, retVal);
                }
                currentScore += retVal;
            }
            if (riskAttributeCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkRiskAttribute: returns {}", ADAPTIVE, retVal);
                }
                currentScore += retVal;
            }
            if (authFailureCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkAuthFailure: returns {}", ADAPTIVE, retVal);
                }
                currentScore += retVal;
            }
            if (deviceCookieCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug( "{}.checkRegisteredClient: returns {}", ADAPTIVE, retVal);
                }
                currentScore += retVal;
            }
            if (geoLocationCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkGeoLocation: returns {}", ADAPTIVE, retVal);
                }
                currentScore += retVal;
            }
            if (reqHeaderCheck) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkRequestHeader: returns {}", ADAPTIVE, retVal);
                }
                currentScore += retVal;
            }
        } catch (Exception ex) {
        ...
        if (currentScore < adaptiveThreshold) {
            if (debug.isDebugEnabled()) {
                debug.debug("{}: Returning Success. Username='{}'", ADAPTIVE, userName);
            }
            return ISAuthConstants.LOGIN_SUCCEED;
        } else {
            if (debug.isDebugEnabled()) {
                debug.debug("{}: Returning Fail. Username='{}'", ADAPTIVE, userName);
            }
    ...
    private Map<String, String> checkIPVersion(String range) {
        ...
        if (range.contains("-")) {
            ...
            debug.debug("IPRange found single IP");
    ...
    protected int checkAuthFailure() {
        ...
        try {
            ...
            if (debug.isWarnEnabled()) {
                debug.warn("{}.checkAuthFailure() : Failed to get fail count", ADAPTIVE, e);
            }
    ...
    protected int checkIPRange() {
        ...
        for (String nextIP : IPRangeRange) {
            ...
            try {
                ...
                if (debug.isWarnEnabled()) {
                    debug.warn("{}.checkIPRange: IP type could not be validated. IP={}", ADAPTIVE, nextIP, e);
                }
                continue;
            }
            ...
            if (ipVersion.equalsIgnoreCase(IP_V6) && ValidateIPaddress.isIPv6(clientIP)) {
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkIPRange: {} --> {}", ADAPTIVE, clientIP, nextIP);
                    debug.debug("IP version is: {}", IP_V6);
                    debug.debug("Client IP is: {}", IPv6Address.fromString(clientIP));
                }
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkIPRange: {} --> {}", ADAPTIVE, clientIP, nextIP);
                    debug.debug("IP version is: {}", IP_V4);
                    debug.debug("Client IP is: {}", clientIP);
                }
    ...
    protected int checkIPHistory() {
        ...
        if (IPHistoryAttribute != null) {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("{}.checkIPHistory: Client IP = {}, History IP = {}", ADAPTIVE, clientIP,
                        ipHistoryValues);
            }
    ...
    protected int checkGeoLocation() {
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("{}.checkGeoLocation: GeoLocation database location = {}", ADAPTIVE, geoLocationDatabase);
        }
        ...
        try {
            ...
            if (debug.isWarnEnabled()) {
                debug.warn("{}.checkGeoLocation: #getCountryCode :: An IO error happened", ADAPTIVE, e);
            }
            return geoLocationScore;
        } catch (GeoIp2Exception e) {
            if (debug.isWarnEnabled()) {
                debug.warn("{}.checkGeoLocation: #getCountryCode :: An error happened when looking up the IP", ADAPTIVE, e);
            }
            return geoLocationScore;
        }

        if (StringUtils.isEmpty(countryCode)) {
            debug.debug("{}.checkGeoLocation: #getCountryCode {} returns countryCode as empty or null",
                    ADAPTIVE, clientIP);
            return geoLocationScore;
        }

        debug.debug("{}.checkGeoLocation: #getCountryCode {} returns {}", ADAPTIVE, clientIP, countryCode);
        StringTokenizer st = new StringTokenizer(geoLocationValues, "|");
        while (st.hasMoreTokens()) {
            if (countryCode.equalsIgnoreCase(st.nextToken())) {
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.checkGeoLocation: Found Country Code : {}", ADAPTIVE, countryCode);
                }
    ...
    protected int checkKnownCookie() {
        int retVal = 0;
        debug.debug("{}.checkKnownCookie:", ADAPTIVE);
    ...
    protected int checkRequestHeader() {
        int retVal = 0;
        debug.debug("{}.checkRequestHeader:", ADAPTIVE);
        ...
        if (req != null) {
            ...
            while (eHdrs.hasMoreElements()) {
                ...
                if (reqHeaderName.equalsIgnoreCase(header)) {
                    if (debug.isDebugEnabled()) {
                        debug.debug("{}.checkRequestHeader: Found header: {}", ADAPTIVE, header);
                    }
                    if (reqHeaderValue != null) {
                        ...
                        while (eVals.hasMoreElements()) {
                            ...
                            if (reqHeaderValue.equalsIgnoreCase(val)) {
                                if (debug.isDebugEnabled()) {
                                    debug.debug("{}.checkRequestHeader: Found header Value: {}", ADAPTIVE,  val);
                                }
    ...
    protected int checkRegisteredClient() {
        int retVal = 0;
        debug.debug("{}.checkRegisteredClient:", ADAPTIVE);
        ...
        if (req != null) {
            ...
            String cookieValue = CookieUtils.getCookieValueFromReq(req, deviceCookieName);
                ...
                if (lastLogin != null) {
                    ...
                    if (lastLogin != null) {
                        try {
                            ...
                            if (debug.isDebugEnabled()) {
                                debug.debug("{}.checkLastLogin: lastLogin '{}' can't be parsed", ADAPTIVE,
                                        lastLogin, pe);
                            }
                        }
                    }
    ...
    protected int checkRiskAttribute() {
        int retVal = 0;
        if (debug.isDebugEnabled()) {
            debug.debug("{}.checkRiskAttribute", ADAPTIVE);
        }
            ...
            if (riskAttributeValues != null) {
                for (String riskAttr : riskAttributeValues) {
                    if (riskAttributeValue.equalsIgnoreCase(riskAttr)) {
                        if (debug.isDebugEnabled()) {
                            debug.debug("{}.checkRiskAttribute: Found Match", ADAPTIVE);
                        }
    ...
    private Set<String> getIdentityAttributeSet(String attr) {
        ...
        try {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("{}.getIdentityAttributeSet Attribute: {}", ADAPTIVE, attr, e);
            }
        }
        return retVal;
    }
    ...
    private String getIdentityAttributeString(String attr) {
        ...
        try {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("{}.getIdentityAttributeString Attribute: {}", ADAPTIVE, attr, e);
            }
        }
        return retVal;
    ...
    private AMIdentity getIdentity() {
        ...
        try {
            ...
            if (searchResults.getSearchResults().isEmpty() && !userSearchAttributes.isEmpty()) {
                if (debug.isDebugEnabled()) {
                    debug.debug("{}.getIdentity : searching user identity with alternative attributes {}", ADAPTIVE,
                            userSearchAttributes);
                }
    ...
    protected void setPostAuthNParams() {
        try {
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("{} Unable to Set PostAuthN Params", ADAPTIVE, e);
            }
        }
    }
    ...
    protected int getOptionAsInteger(Map m, String i) {
        ...
        if (s != null) {
            try {
                ...
                debug.debug("{}.getOptionAsInteger() : The value of '{}' is not parsable integer: '{}'", ADAPTIVE, i, s);
            }
        }
        return retVal;
    }
    ...
    private static synchronized DatabaseReader getLookupService(String dbLocation) {
        try {
            ...
            debug.debug("{}.getLookupService : Unable to initialize GeoDB service: {}", ADAPTIVE, ioe.getMessage());
        }
