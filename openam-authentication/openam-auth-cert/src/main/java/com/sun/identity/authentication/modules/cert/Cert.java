...
public class Cert extends AMLoginModule {
    ...
    public void init(Subject subject, Map sharedState, Map options) {
        if (debug == null) {
            debug = org.slf4j.LoggerFactory.getLogger(amAuthCert);
        }
        ...
        if (debug.isDebugEnabled()) {
            debug.debug("Cert Auth resbundle locale="+locale);
            debug.debug("Cert auth init() done");
        }
    }

    private void initAuthConfig() throws AuthLoginException {
        if (options != null) {
            debug.debug("Certificate: getting attributes.");
            ...
            if (gwCertAuth == null || gwCertAuth.equals("")
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("iplanet-am-auth-cert-gw-cert-auth-enabled = "
                        + gwCertAuth);
                }
                ...
                if ((client !=null) && (portalGateways.contains(client))) {
                    ...
                    if (debug.isDebugEnabled()) {
                        debug.debug("gateway list does not contain client");
                        Iterator clientIter = portalGateways.iterator();
                        while (clientIter.hasNext()) {
                            String clientStr = (String)clientIter.next();
                            debug.debug("client list entry = " + clientStr);
                        }
                    }
                 }
            }
    ...
    public int process (Callback[] callbacks, int state)
        ...
        try {
            ...
            if (servletRequest != null) {
                ...
                if (allCerts == null || allCerts.length == 0) {
                    ...
                    if (portal_gw_cert_auth_enabled && portal_gw_cert_preferred) {
                       ...
                       if (debug.isDebugEnabled()) {
                           debug.debug("Certificate: got all certs from " +
            ...
            if (thecert == null) {
                debug.debug("Certificate: no cert passed in.");
            ...
            if(debug.isDebugEnabled()){
                debug.debug("in Certificate. userTokenId=" +
    ...
    private int doJCERevocationValidation(X509Certificate[] allCerts)
    	...
    	try {
            ...
            if (!certpath.verify(allCerts, crlEnabled, ocspEnabled)) {
                ...
                if (debug.isDebugEnabled()) {
                    debug.debug("CertPath:verify success.");
                }
            }
        ...
        try {
            ...
            for (List<?> subjectAlternativeName : subjectAlternativeNames) {
                ...
                case 0: // otherName
                    ...
                    break;
                case 1: // rfc822Name
                    if (amAuthCert_subjectAltExtMapper.equalsIgnoreCase("RFC822Name")) {
                        userTokenId = subjectAlternativeName.get(1).toString();
                    }
                    break;
                default:
                    // Skip
     ...
     */
         ...
         */
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("getTokenFromCert: Subject DN : " + CertUtils.getSubjectName(cert));
            }
            ...
            if (debug.isDebugEnabled()) {
                debug.debug("getTokenFromCert: " + amAuthCert_userProfileMapper + " " + userTokenId);
            }
        } catch (Exception e) {
            if (debug.isDebugEnabled()) {
                debug.debug("Certificate - Error in getTokenFromSubjectDN = " , e);
            }
            throw new AuthLoginException(amAuthCert, "CertNoReg", null);
        }
    }
    ...
    private X509Certificate sendCallback() throws AuthLoginException {
        ...
        try {
            ...
            debug.debug("message type missing");
    ...
    private X509Certificate getPortalStyleCert (HttpServletRequest request)
       ...
       if ((certParamName != null) && (certParamName.length() > 0)) {
           debug.debug ("getPortalStyleCert: checking cert in HTTP header");
           ...
           debug.debug("getPortalStyleCert: checking cert in userCert param");
       ...
       if (debug.isDebugEnabled()) {
           debug.debug ("in Certificate. validate certParam: " + certParam);
       }
       if (certParam == null || certParam.equals("")) {
           debug.debug("Certificate: no cert from HttpServletRequest header");
           throw new AuthLoginException(amAuthCert, "noCert", null);
       }

       byte[] decoded = Base64.decode(Uris.urlDecodeFragment(certParam));
       if (decoded == null) {
       ...
       debug.debug("Certificate: CertificateFactory.getInstance.");
       ...
       if (debug.isDebugEnabled()) {
           debug.debug("X509Certificate: principal is: " +
